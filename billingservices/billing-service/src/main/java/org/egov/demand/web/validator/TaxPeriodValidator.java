...
public class TaxPeriodValidator {
    ...
    public void validateTaxPeriods(TaxPeriodRequest taxPeriodRequest, String mode) {
        List<TaxPeriod> taxPeriodList = taxPeriodRequest.getTaxPeriods();
        if ("edit".equalsIgnoreCase(mode)) {
            for (TaxPeriod taxPeriod : taxPeriodList) {
                if (StringUtils.isBlank(taxPeriod.getId()))
                    throw new RuntimeException("Id cannot be null, Please provide the Id ");
            }
        }
        boolean result = taxPeriodRepository.checkForDuplicates(taxPeriodList, mode);
        if (result)
            throw new RuntimeException("Tax Period already exists for this combination of tenantId, service and code ");
