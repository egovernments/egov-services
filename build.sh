...
pwd=`pwd`
ci=`sudo docker run -d -v $pwd:/var/lib/egov-services:rw -w /var/lib/egov-services --net=container:${db} ${ci_image} /bin/bash -c "cd $1;mvn clean verify package -s /var/lib/egov-services/settings.xml; cp ../../newrelic/newrelic.* target/."`
