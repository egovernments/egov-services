...
public class StatusQueryBuilder {
	private static final Logger logger = LoggerFactory.getLogger(StatusQueryBuilder.class);

	private static final String BASE_QUERY = "select id,code,objecttype,description,tenantId,createdBy,"
			+ "createdDate,lastModifiedBy,lastModifiedDate FROM egcl_status";

	@SuppressWarnings("rawtypes")
	public String getQuery(StatusCriteria criteria, List preparedStatementValues) {
	...
	private void addWhereClause(StringBuilder selectQuery, List preparedStatementValues,
		...
		if (criteria.getCode() != null) {
			...
			preparedStatementValues.add(criteria.getCode());
		}

		if (criteria.getObjectType() != null) {
			isAppendAndClause = addAndClauseIfRequired(isAppendAndClause, selectQuery);
			selectQuery.append(" objecttype = ?");
			preparedStatementValues.add(criteria.getObjectType());
		}
	}
