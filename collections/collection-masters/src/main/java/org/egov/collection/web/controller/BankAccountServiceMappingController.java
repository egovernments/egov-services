...
public class BankAccountServiceMappingController {
    ...
    public ResponseEntity<?> create(@RequestBody BankAccountServiceMappingReq bankAccountServiceMappingReq, final BindingResult errors) {

        log.info("BankAccountServiceMapping Request::" + bankAccountServiceMappingReq);
        final ErrorResponse errorResponse = collectionMastersRequestValidator.validateBankAccountServiceRequest(bankAccountServiceMappingReq);
        if (errorResponse != null)
            return new ResponseEntity<ErrorResponse>(errorResponse, HttpStatus.BAD_REQUEST);
        BankAccountServiceMappingReq bankAccountServiceReq =  bankAccountMappingService.createBankAccountServiceMappingAsync(bankAccountServiceMappingReq);
    ...
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<?> search(@ModelAttribute BankAccountServiceMappingSearchReq bankAccountServiceMappingSearchReq,final BindingResult modelAttributeBindingResult,
                                    @RequestBody @Valid final RequestInfoWrapper requestInfoWrapper) {

        log.info("Search request for BankAccount service mapping :: "+bankAccountServiceMappingSearchReq);

        BankAccountServiceMappingSearchCriteria searchCriteria = BankAccountServiceMappingSearchCriteria.builder().bankAccount(bankAccountServiceMappingSearchReq.getBankAccount())
                .businessDetails(bankAccountServiceMappingSearchReq.getBusinessDetails()).tenantId(bankAccountServiceMappingSearchReq.getTenantId()).build();
        ...
        List<org.egov.collection.domain.model.BankAccountServiceMapping> bankAccountServiceMappings = bankAccountMappingService.searchBankAccountService(searchCriteria);

        List<BankAccountServiceMapping> contractBankServiceMappings = new BankAccountServiceMapping().toContractList(bankAccountServiceMappings);
