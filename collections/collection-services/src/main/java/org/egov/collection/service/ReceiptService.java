...
public class ReceiptService {
	...
	public Receipt pushToQueue(ReceiptReq receiptReq) {
		...
		for (BillDetail billdetails : receiptReq.getReceipt().getBill().getBillDetails()) {
			...
			logger.info("FUND: " + fund + " FUNDSOURCE: " + fundSource + " FUNCTION: " + function + " DEPARTMENT: "
					+ department);
	...
	public Receipt create(ReceiptReq receiptReq) {
		...
		for (BillDetail billdetails : receiptInfo.getBill().getBillDetails()) {
			...
			Map<String, Object>[] parametersReceiptDetails = (Map<String, Object>[]) new Map[billdetails
	...
	public BusinessDetailsResponse getBusinessDetails(String businessDetailsCode, ReceiptReq receiptReq) {
		...
		String searchCriteria = "?businessDetailsCode=" + businessDetailsCode + "&tenantId="
				+ receiptReq.getReceipt().getTenantId();
		...
		try {
			businessDetailsResponse = restTemplate.postForObject(builder.toString(), requestInfoWrapper,
	...
	/*
	 * private String getStatusCode(RequestInfo requestInfo){ logger.info(
	 * "fetching status for the receipt.");
	 * 
	 * StringBuilder builder = new StringBuilder(); String baseUri =
	 * CollectionServiceConstants.STATUS_SEARCH_URI; String searchCriteria=
	 * "?objectType=ReceiptHeader&tenantId=default&code=SUBMITTED";
	 * builder.append(baseUri).append(searchCriteria);
	 * 
	 * logger.info("URI being hit: "+builder.toString());
	 * 
	 ...
	 * "Error while fecthing COAs for validation from financial service. "
	 * +e.getCause()); } logger.info("Response from collection-masters: "
	 * +response.toString());
	 * 
	...
	public String generateReceiptNumber(ReceiptReq receiptRequest) {
		...
		idGenReq.setTs(receiptRequest.getRequestInfo().getTs().getTime()); // this
																			// is
																			// the
																			// difference.
	...
	public Boolean checkVoucherCreation(Boolean voucherCreation, Date voucherCutoffDate, Date receiptDate) {
		Boolean createVoucherForBillingService = Boolean.FALSE;
		if (voucherCutoffDate != null && receiptDate.compareTo(voucherCutoffDate) > 0) {
