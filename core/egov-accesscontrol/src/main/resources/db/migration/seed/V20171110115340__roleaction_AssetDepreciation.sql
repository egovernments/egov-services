...
insert into eg_action(id,name,url,servicecode,queryparams,parentmodule,ordernumber,displayname,enabled,createdby,createddate,lastmodifiedby,lastmodifieddate)values(nextval('SEQ_EG_ACTION'),'Generate Depreciation','/app/asset/asset-depreciation.html','AssetTransactions',null,(select id from service where name='Asset Transactions' and tenantId='default'),3,'Generate Depreciation',true,1,now(),1,now());
