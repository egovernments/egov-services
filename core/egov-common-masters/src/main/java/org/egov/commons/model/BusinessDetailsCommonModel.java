...
public class BusinessDetailsCommonModel {
	...
	public List<BusinessDetailsRequestInfo> toDomainContract() {
		...
		for(BusinessDetails details:businessDetails){
			BusinessDetailsRequestInfo detailsRequestInfo=BusinessDetailsRequestInfo.builder().id(details.getId()).active(details.getIsEnabled()).code(details.getCode()).
					name(details.getName()).businessCategory(details.getBusinessCategory().getId()).businessType(details.getBusinessType())
					.businessUrl(details.getBusinessUrl()).department(details.getDepartment()).fundSource(details.getFundSource()).
					function(details.getFunction()).functionary(details.getFunctionary()).fund(details.getFund()).isVoucherApproved
					(details.getIsVoucherApproved()).ordernumber(details.getOrdernumber()).tenantId(details.getTenantId()).
					voucherCreation(details.getVoucherCreation()).voucherCutoffDate(details.getVoucherCutoffDate()).build();
			org.egov.commons.web.contract.BusinessDetails detail=org.egov.commons.web.contract.BusinessDetails.builder().id(details.getId()).active(details.getIsEnabled()).name(details.getName()).
			businessCategory(details.getBusinessCategory().getId()).businessType(details.getBusinessType())
			.businessUrl(details.getBusinessUrl()).department(details.getDepartment()).fundSource(details.getFundSource()).
			function(details.getFunction()).functionary(details.getFunctionary()).fund(details.getFund()).isVoucherApproved
			(details.getIsVoucherApproved()).ordernumber(details.getOrdernumber()).tenantId(details.getTenantId()).
			voucherCreation(details.getVoucherCreation()).voucherCutoffDate(details.getVoucherCutoffDate()).code(details.getCode()).build();
            ...
            for(BusinessAccountDetails accountDetails:businessAccountDetails){
              ...
              for(BusinessAccountDetails requiredAccount:requiredBusinessAccountDetails){
            	  contractAccountDetails.add(org.egov.commons.web.contract.BusinessAccountDetails.builder().id(requiredAccount.getId()).amount(requiredAccount.getAmount())
            ...
            for(BusinessAccountSubLedgerDetails subledgerDetails:businessAccountSubledgerDetails){
            	if(subledgerDetails.getBusinessAccountDetail().getId().equals(account.getId())){
            		contractAccountSubledger.add(BusinessAccountSubLedger.builder().id(subledgerDetails.getId()).amount(subledgerDetails.getAmount()).
        		detailKey(subledgerDetails.getAccountDetailKey()).detailType(subledgerDetails.getAccountDetailType()).
        		businessAccountDetails(account).build());
         }
	}
            }
