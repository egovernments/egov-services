...
public class BusinessDetailsRepository {
	...
	public static final String UPDATE_BUSINESS_ACCOUNT_DETAILS="Update eg_business_accountdetails"
			+ " set businessDetails=?,chartOfAccount=?,amount=?,tenantId=?"
			+ " where id=?";
	...
	public static final String GET_BUSINESS_ACCOUNTDETAILS_BY_BUSINESSDETAILS="Select * from"
			+ " eg_business_accountdetails where businessDetails=? and"
			+ " tenantId=?";
		...
		for (BusinessAccountSubLedgerDetails subledger : listModelAccountSubledger) {
			for (BusinessAccountDetails accountDetails : accountDetailsFromDB) {
				if (subledger.getBusinessAccountDetail().getChartOfAccount()
					...
					Object[] subledgerRecord = { generateSequence(SEQUENCEFORSUBLEDGER),
							subledger.getAmount(), accountDetails.getId(), subledger.getAccountDetailKey(),
							subledger.getAccountDetailType(), subledger.getTenantId() };
					batch.add(subledgerRecord);
				}
			}
		}
		jdbcTemplate.batchUpdate(INSERT_BUSINESS_ACCOUNT_SUBLEDGER_DETAILS, batch);
		return new BusinessDetailsCommonModel(Collections.singletonList(modelDetails), listModelAccountDetails, listModelAccountSubledger);
	}
	...
	public BusinessDetailsCommonModel updateBusinessDetails(BusinessDetails modelDetails,
      ...
      Object[] obj = new Object[] {
    		    modelDetails.getName(), modelDetails.getIsEnabled(),
				modelDetails.getCode(), modelDetails.getBusinessType(), modelDetails.getBusinessUrl(),
				modelDetails.getVoucherCutoffDate(), modelDetails.getOrdernumber(), modelDetails.getVoucherCreation(),
				modelDetails.getIsVoucherApproved(), modelDetails.getFund(), modelDetails.getDepartment(),
				modelDetails.getFundSource(), modelDetails.getFunctionary(), modelDetails.getBusinessCategory().getId(),
	...
	List<BusinessAccountDetails>accountDetailsFromDB=jdbcTemplate.query(GET_BUSINESS_ACCOUNTDETAILS_BY_BUSINESSDETAILS,preparedStatementValue.toArray(),
				businessAccountDetailsRowMapper);
 HashMap<Long,Long> mapOfInsertedIdsInModelAndInDB=new HashMap<>();
	...
	List<Long> deleteAccountDetailsId=deletedAccountDetails.stream().map(accountdetail->accountdetail.getId()).collect(Collectors.toList());
	if(!deleteAccountDetailsId.isEmpty())
		deleteAccountDetails(deleteAccountDetailsId,modelDetails.getTenantId());
	List<Long> accountDetailsIds=listModelAccountDetails.stream().map(modelAccountDetail->modelAccountDetail.getId()).collect(Collectors.toList());
    Map<String, Object> namedParameters = new HashMap<>();
	namedParameters.put("id",accountDetailsIds);
    namedParameters.put(TENANT,listModelAccountDetails.get(0).getTenantId());
    
    List<BusinessAccountSubLedgerDetails> subledgerfromDB=namedParameterJdbcTemplate.query(GET_BUSINESS_ACCOUNT_SUBLEDGER_DETAILS
    		,namedParameters,businessAccountSubledgerDetailsRowMapper);
    ...
    List<Long> deleteSubledgerDetailsIds= accountSubledgerToBeDeleted.stream().map(subledger->subledger.getId()).collect(Collectors.toList());
	...
	private List<BusinessAccountSubLedgerDetails> deleteSubledgerDetailsFromDBIfNotPresentInInput(List<BusinessAccountSubLedgerDetails> subledgerfromDB,
	...
	private void updateSubledgerDetails(BusinessAccountSubLedgerDetails subledgerModel) {
		 Object obj[]= new Object[]{subledgerModel.getAmount(),subledgerModel.getBusinessAccountDetail().getId(),
				 subledgerModel.getAccountDetailKey(),subledgerModel.getAccountDetailType(),subledgerModel.getTenantId(),
				 subledgerModel.getId()};
	...
	private void insertSubledgerDetails(BusinessAccountSubLedgerDetails subledgerModel,
       ...
       if(mapOfInsertedIdsInModelAndInDB.containsKey(subledgerModel.getBusinessAccountDetail().getId())){
		  for (Map.Entry<Long, Long> entry : mapOfInsertedIdsInModelAndInDB.entrySet()) {
				if(entry.getKey().equals(subledgerModel.getBusinessAccountDetail().getId())){
				 ...
				 Object obj[]= new Object[]{generateSequence(SEQUENCEFORSUBLEDGER),
							subledgerModel.getAmount(),businessDetailsId,
							subledgerModel.getAccountDetailKey(),subledgerModel.getAccountDetailType(),subledgerModel.getTenantId()};
       ...
       Object obj[]= new Object[]{generateSequence("SEQ_EG_BUSINESS_SUBLEDGERINFO"),
				subledgerModel.getAmount(),businessDetailsId,
				subledgerModel.getAccountDetailKey(),subledgerModel.getAccountDetailType(),subledgerModel.getTenantId()};
				jdbcTemplate.update(INSERT_BUSINESS_ACCOUNT_SUBLEDGER_DETAILS,obj);
    	   }
	...
	private List<BusinessAccountDetails> deleteAccountDetailsFromDBIfNotPresentInInput(
			List<BusinessAccountDetails> listModelAccountSubledger,
    ...
    private void updateAccountDetails(BusinessAccountDetails accountdetail) {
		Object[] object=new Object[]{accountdetail.getBusinessDetails().getId(),
				accountdetail.getChartOfAccount(),accountdetail.getAmount(),accountdetail.getTenantId(),accountdetail.getId()};
	...
	public BusinessDetailsCommonModel getForCriteria(BusinessDetailsCriteria detailsCriteria) {
		...
		List<BusinessDetails> details=jdbcTemplate.query(qryString, preparedStatementValues.toArray(),businessDetailsCombinedRowMapper);
	 ...
	 List<BusinessDetails> uniqueBusinessDetails=details.stream().collect(collectingAndThen(toCollection(() -> new TreeSet<>(comparingLong(BusinessDetails::getId))),
             ArrayList::new));
	 List<BusinessAccountDetails> uniqueBusinessAccountDetails=accountDetails.stream().filter(accountdetail->accountdetail.getId()!=null).
			 collect(collectingAndThen(toCollection(() -> new TreeSet<>(comparingLong(BusinessAccountDetails::getId))),ArrayList::new));
	 List<BusinessAccountSubLedgerDetails> uniqueBusinessAccountSubledgerDetails=subledgerDetails.stream().filter(subledgerdetail->subledgerdetail.getId()!=null).
			 collect(collectingAndThen(toCollection(() -> new TreeSet<>(comparingLong(BusinessAccountSubLedgerDetails::getId))),ArrayList::new));
   
	List<BusinessDetails> businessDetails= uniqueBusinessDetails.stream().map(unqdetail->unqdetail.toDomainModel()).collect(Collectors.toList());
	List<BusinessAccountDetails> businessAccountDetails=uniqueBusinessAccountDetails.stream().map(unqAccDetail->unqAccDetail.toDomainModel()).collect(Collectors.toList());
	 List<BusinessAccountSubLedgerDetails>businessAccountSubledger= uniqueBusinessAccountSubledgerDetails.stream().map(unqAccountDetail->unqAccountDetail.toDomainModel()).collect(Collectors.toList());
