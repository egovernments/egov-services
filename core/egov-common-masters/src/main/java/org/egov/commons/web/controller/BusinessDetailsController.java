...
public class BusinessDetailsController {
	...
	public ResponseEntity<?> createBusinessDetails(@RequestBody BusinessDetailsRequest businessDetailsRequest, final BindingResult errors) {
		...
		BusinessCategory modelCategory=businessCategoryService.getBusinessCategoryByIdAndTenantId(detailsInfo
				.getBusinessCategory(),detailsInfo.getTenantId());
        ...
        listModelAccountDetails.add(new org.egov.commons.model.BusinessAccountDetails(details,modelDetails,false));
	    }
		...
		List<BusinessDetailsRequestInfo> detailsRequestInfo=businessDetailsService.createBusinessDetails(modelDetails,listModelAccountDetails,
				listModelAccountSubledger,user).toDomainContract();
	...
	public ResponseEntity<?> updateBusinessDetails(@RequestBody BusinessDetailsRequest businessDetailsRequest,
		...
		List<BusinessDetailsRequestInfo> detailsRequestInfo=businessDetailsService.updateBusinessDetails(modelDetails,listModelAccountDetails,
				listModelAccountSubledger,user).toDomainContract();
		return  getSuccessResponse(businessDetailsRequest.getRequestInfo(),detailsRequestInfo);
 }
	...
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<?> searchBusinessDetails(@ModelAttribute @Valid final BusinessDetailsGetRequest detailsGetRequest,
    	...
    	BusinessDetailsCriteria detailsCriteria=BusinessDetailsCriteria.builder().
    			...
    			.sortBy(detailsGetRequest.getSortBy()).sortOrder(detailsGetRequest.getSortOrder()).tenantId(
    					detailsGetRequest.getTenantId()).build();
    ...
    private ResponseEntity<?> getSuccessResponse(RequestInfo requestInfo,List<BusinessDetailsRequestInfo> detailsRequestInfo) {
    ...
    private Error getError(final BusinessDetailsRequest businessDetailsRequest) {
        ...
        return Error.builder().code(HttpStatus.BAD_REQUEST.value())
                .message(CollectionConstants.INVALID_DETAILS_REQUEST_MESSAGE)
		 ...
		 final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
	        if (detailsInfo.getBusinessCategory() == null) {
	            final ErrorField errorField = ErrorField.builder()
	                    .code(CollectionConstants.DETAILS_CATEGORY_MANDATORY_CODE)
	                    .message(CollectionConstants.DETAILS_CATEGORY_MANADATORY_ERROR_MESSAGE)
	                    .field(CollectionConstants.DETAILS_CATEGORY_MANADATORY_FIELD_NAME)
	                    .build();
	...
	private void addFundValidationErrors(BusinessDetailsRequest businessDetailsRequest, List<ErrorField> errorFields) {
		 final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
	        if (detailsInfo.getFund() == null || detailsInfo.getFund().isEmpty()) {
	            final ErrorField errorField = ErrorField.builder()
	                    .code(CollectionConstants.DETAILS_FUND_MANDATORY_CODE)
	                    .message(CollectionConstants.DETAILS_FUND_MANADATORY_ERROR_MESSAGE)
	                    .field(CollectionConstants.DETAILS_FUND_MANADATORY_FIELD_NAME)
	                    .build();
	            errorFields.add(errorField);
	        } else
	            return;
	...
	private void addFunctionValidationErrors(BusinessDetailsRequest businessDetailsRequest, List<ErrorField> errorFields) {
		 final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
	        if (detailsInfo.getFunction() == null || detailsInfo.getFunction().isEmpty()) {
	            final ErrorField errorField = ErrorField.builder()
	                    .code(CollectionConstants.DETAILS_FUNCTION_MANDATORY_CODE)
	                    .message(CollectionConstants.DETAILS_FUNCTION_MANADATORY_ERROR_MESSAGE)
	                    .field(CollectionConstants.DETAILS_FUNCTION_MANADATORY_FIELD_NAME)
	                    .build();
	            errorFields.add(errorField);
	        } else
	            return;
	...
	private void addBusinessTypeValidationErrors(BusinessDetailsRequest businessDetailsRequest,
		 ...
		 final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
	        if (detailsInfo.getBusinessType() == null || detailsInfo.getBusinessType().isEmpty()) {
	            final ErrorField errorField = ErrorField.builder()
	                    .code(CollectionConstants.DETAILS_TYPE_MANDATORY_CODE)
	                    .message(CollectionConstants.DETAILS_TYPE_MANADATORY_ERROR_MESSAGE)
	                    .field(CollectionConstants.DETAILS_TYPE_MANADATORY_FIELD_NAME)
	                    .build();
	            errorFields.add(errorField);
	        } else
	            return;
	...
	private void addTenantIdValidationErrors(final BusinessDetailsRequest businessDetailsRequest, final List<ErrorField> errorFields) {
        final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
        if (detailsInfo.getTenantId() == null || detailsInfo.getTenantId().isEmpty()) {
            final ErrorField errorField = ErrorField.builder()
                    .code(CollectionConstants.TENANT_MANDATORY_CODE)
                    .message(CollectionConstants.TENANT_MANADATORY_ERROR_MESSAGE)
                    .field(CollectionConstants.TENANT_MANADATORY_FIELD_NAME)
                    .build();
            errorFields.add(errorField);
        } else
            return;
	 ...
	 private void addNameValidationErrors(final BusinessDetailsRequest businessDetailsRequest, final List<ErrorField> errorFields) {
		 final BusinessDetailsRequestInfo detailsInfo = businessDetailsRequest.getBusinessDetails();
        if (detailsInfo.getName() == null|| detailsInfo.getName().isEmpty()) {
            final ErrorField errorField = ErrorField.builder()
                    .code(CollectionConstants.DETAILS_NAME_MANDATORY_CODE)
                    .message(CollectionConstants.DETAILS_NAME_MANADATORY_ERROR_MESSAGE)
                    .field(CollectionConstants.DETAILS_NAME_MANADATORY_FIELD_NAME)
                    .build();
            errorFields.add(errorField);
        }
         else if(!businessDetailsService.getBusinessDetailsByNameAndTenantId(detailsInfo.getName(), detailsInfo.getTenantId()))
        		{
        	final ErrorField errorField=ErrorField.builder()
        			.code(CollectionConstants.DETAILS_NAME_UNIQUE_CODE)
        			.message(CollectionConstants.DETAILS_NAME_UNIQUE_ERROR_MESSAGE)
        			.field(CollectionConstants.DETAILS_NAME_UNIQUE_FIELD_NAME)
        			.build();
        	errorFields.add(errorField);
        	}
        else
        ...
        private void addCodeValidationErrors(final BusinessDetailsRequest businessDetailsRequest,
             ...
             if (detailsInfo.getCode() == null|| detailsInfo.getCode().isEmpty()) {
                 final ErrorField errorField = ErrorField.builder()
                         .code(CollectionConstants.DETAILS_CODE_MANDATORY_CODE)
                         .message(CollectionConstants.DETAILS_CODE_MANADATORY_ERROR_MESSAGE)
                         .field(CollectionConstants.DETAILS_CODE_MANADATORY_FIELD_NAME)
                         .build();
                 errorFields.add(errorField);
             }else if(!businessDetailsService.getBusinessDetailsByCodeAndTenantId(detailsInfo.getCode(), detailsInfo.getTenantId()))
             		{
             	final ErrorField errorField=ErrorField.builder()
             			.code(CollectionConstants.DETAILS_CODE_UNIQUE_CODE)
             			.message(CollectionConstants.DETAILS_CODE_UNIQUE_ERROR_MESSAGE)
             			.field(CollectionConstants.DETAILS_CODE_UNIQUE_FIELD_NAME)
             			.build();
             	errorFields.add(errorField);
             	}
             else
