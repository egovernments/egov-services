...
public class BusinessCategoryControllerTest {
	...
	public void test_should_create_business_category() throws Exception {
		...
		when(serviceCategoryService.create(getBusinessCategoryModel(),getAuthenticatedUser())).thenReturn(getBusinessCategoryModel());
      
		mockMvc.perform(post("/businessCategory/_create").contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(getFileContents("businessCategoryRequestCreate.json")))
	...
	public void test_should_update_business_category() throws Exception {
		 when(responseInfoFactory.createResponseInfoFromRequestInfo(getRequestInfo(),true)).thenReturn(getResponseInfo());
	        when(serviceCategoryService.getBusinessCategoryByNameAndTenantId("Collection", "default")).thenReturn(true);
	       when(serviceCategoryService.getBusinessCategoryByCodeAndTenantId("CL", "default")).thenReturn(true);
		when(serviceCategoryService.update("CLL",getBusinessCategoryModel(),getAuthenticatedUser())).thenReturn(getBusinessCategoryModel());
	...
	public void test_should_get_all_business_categories_with_the_params_criteria() throws Exception {
		 when(responseInfoFactory.createResponseInfoFromRequestInfo(getRequestInfo(),true)).thenReturn(getResponseInfo());
		when(serviceCategoryService.getForCriteria(getBusinessCriteria())).thenReturn(getListOfModelBusinessCategories());
	
		mockMvc.perform(post("/businessCategory/_search?active=true&tenantId=default&ids=1,2,3&sortBy=code&sortOrder=desc")
						.contentType(MediaType.APPLICATION_JSON_UTF8)
  ...
  private BusinessCategoryCriteria getBusinessCriteria() {
     ...
     private RequestInfo getRequestInfo() {
		UserInfo userInfo=UserInfo.builder().id(1L).name("ram").emailId("ram@gmail.com").mobileNumber("73878921").build();
		return RequestInfo.builder().apiId("org.egov.collection").ver("1.0").action("POST").did("4354648646")
				.key("xyz").msgId("654654").requesterId("61").authToken("345678f").userInfo(userInfo).build();   
	}
