...
public class BusinessDetailsControllerTest {
	...
	public void test_should_create_business_details() throws Exception {
		  when(responseInfoFactory.createResponseInfoFromRequestInfo(getRequestInfo(),true)).thenReturn(getResponseInfo());
		...
		when(businessCategoryService.getBusinessCategoryByIdAndTenantId(1L,"default")).thenReturn(getBusinessCategoryModel());
		...
		mockMvc.perform(post("/businessDetails/_create").contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(getFileContents("businessDetailsRequestCreate.json")))
	...
	public void test_should_update_business_details() throws Exception {
		  when(responseInfoFactory.createResponseInfoFromRequestInfo(getRequestInfo(),true)).thenReturn(getResponseInfo());
	        when(businessDetailsService.getBusinessDetailsByNameAndTenantId("Trade Licence Mutation", "default")).thenReturn(true);
	       when(businessDetailsService.getBusinessDetailsByCodeAndTenantId("TLM", "default")).thenReturn(true);
		when(businessCategoryService.getBusinessCategoryByIdAndTenantId(1L,"default")).thenReturn(getBusinessCategoryModel());
		when(businessDetailsService.updateBusinessDetails(getModelDetailsForUpdate(), getListOfModelAccountDetailsForUpdate(),
		...
		mockMvc.perform(post("/businessDetails/TL/_update").contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(getFileContents("businessDetailsRequestUpdate.json")))
		        .andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
	...
	public void test_should_search_business_details_based_on_criteria() throws Exception {
		...
		when(businessDetailsService.getForCriteria(getBusinessDetailsCriteria())).thenReturn(getBusinessDetailCommonModel());
		mockMvc.perform(post("/businessDetails/_search?active=true&businessCategoryCode=TL&tenantId=default&ids=1,2&sortBy=code&sortOrder=desc")
	...
	private RequestInfo getRequestInfo() {
			...
			return RequestInfo.builder().apiId("org.egov.collection").ver("1.0").action("POST").did("4354648646")
					.key("xyz").msgId("654654").requesterId("61").authToken("aceb9362-4147-40ff-936a-34497f83d740")
		...
		private ResponseInfo getResponseInfo() {
			return 	ResponseInfo.builder().apiId("org.egov.collection").ver("1.0").resMsgId("uief87324") .msgId("654654").status("successful").build();
	  }
