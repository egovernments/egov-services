...
public class BusinessCategoryRepositoryTest {
	...
	assertTrue(category.equals(businessCategoryRepository.create(getBusinessCategoryModel(),getAuthenticatedUser())));
    ...
    public void test_should_update_serviceCategory_inDB()
	{
		...
		when(jdbcTemplate.query(any(String.class),any(Object[].class),
				any(BusinessCategoryRowMapper.class))).thenReturn(getListOfModelServiceCategories());
		when(jdbcTemplate.update(any(String.class),any (Object[].class))).thenReturn(1);
	    assertTrue(category.equals(businessCategoryRepository.update("CLL",getBusinessCategoryModel(),getAuthenticatedUser())));
    }
	
	
	@Test
    public void test_should_get_All_ServiceCategories_As_per_Criteria(){
	when(businessCategoryQueryBuilder.getQuery(any(BusinessCategoryCriteria.class),any(List.class))).thenReturn("");
	when(jdbcTemplate.query(any(String.class),any(Object[].class),any(BusinessCategoryRowMapper.class))).thenReturn(getListOfModelBusinessCategories());
	List<BusinessCategory>listOfCategories=businessCategoryRepository.getForCriteria(new BusinessCategoryCriteria());
	...
	return Arrays.asList( BusinessCategory.builder().id(5L).code("CLL").name("Collection Label").tenantId("default").build());
    }
