...
public class StateService {
	...
	public List<State> getStates(List<Long> ownerIds, List<String> types, Long userId, String tenantId) {
		...
		if (ownerIds != null && !ownerIds.isEmpty())
			return getSession().createCriteria(State.class).setFlushMode(FlushMode.MANUAL).setReadOnly(true)
					.setCacheable(true).add(Restrictions.in("type", types))
					.add(Restrictions.in("ownerPosition", ownerIds)).add(Restrictions.eq("tenantId", tenantId))
	 ...
	 * stateRepository.findAll(specification, page); }
	 * 
