...
public class DataUploadService {
	...
	public List<UploadJob> createUploadJob(UploaderRequest uploaderRequest) {
		...
		uri.append(fileStoreHost).append(getFileEndpoint).append("?fileStoreId=").append(uploadJob.getRequestFilePath())
				.append("&tenantId=").append(uploadJob.getTenantId());
		try {
			String filePath = dataUploadRepository.getFileContents(uri.toString(),
					uploaderRequest.getUploadJobs().get(0).getRequestFileName());
			...
			AuditDetails auditDetails = AuditDetails.builder()
					.createdBy(uploaderRequest.getRequestInfo().getUserInfo().getId().toString())
			 ...
			 * catch (IOException ioe) { logger.error("Failed to create file", ioe); throw
			 * new CustomException("400", "Unable to create or write file"); }
	...
	private void validateJob(UploadJob uploadJob) {
		...
		Optional<Definition> definitionOptional = runner.getUploadDefinition(uploadJob.getModuleName(),
				uploadJob.getDefName());
	...
	private void uploadFlatData(Document document, Definition uploadDefinition, UploaderRequest uploaderRequest) {
		...
		AuditDetails auditDetails = uploadJob.getAuditDetails();
		List<List<Object>> responseJsonPathLists = initialiseUploadProcess(document, uploadDefinition, uploaderRequest,
				outputHeaders, uploadJob, requests, documentContexts, auditDetails);
		...
		try {
			...
			for (List<Object> row : document.getRows()) {
				...
				for (int i = 0; i < uploadDefinition.getRequests().size(); i++) {
					...
					if (!row.isEmpty()) {
						...
						responseFields.addAll(
								dataUploadUtils.fetchValuesFromResponse(response, responseJsonPathLists.get(i)));
					}
				}
				...
				if ((recordCount % updateProgressSize) == 0) { // update progress after every 'updateProgressSize'
																// records
	...
	private void uploadParentChildData(Document document, Definition uploadDefinition,
		...
		try {
			String resultFilePath = dataUploadUtils.createANewFile(resFilePrefix + uploadJob.getRequestFileName());
			dataUploadUtils.writeToexcelSheet(outputHeaders, resultFilePath);
			// Till now the columnHeaders have been written to result xls. Content
			...
			for (Map.Entry<String, List<List<Object>>> entry : groupedRows.entrySet()) {
				...
				for (int i = 0; i < uploadDefinition.getRequests().size(); i++) {
					...
					String request = buildRequestForParentChild(filteredList, document.getHeaders(), requests.get(i),
							uploadDefinition, documentContexts.get(i), uploaderRequest, previousResponse);
					...
					responseFields
							.addAll(dataUploadUtils.fetchValuesFromResponse(response, responseJsonPathLists.get(i)));
	...
	private List<List<Object>> initialiseUploadProcess(Document document, Definition uploadDefinition,
			UploaderRequest uploaderRequest, List<Object> outputHeaders, UploadJob uploadJob, List<Request> requests,
			List<DocumentContext> documentContexts, AuditDetails auditDetails) {
	...
	private String buildRequest(List<String> columnHeaders, Request request, DocumentContext documentContext,
			UploaderRequest uploaderRequest, List<Object> row, Object previousResponse) {
	...
	private void writeResultToExcel(String failureMessage, List<Object> row, List<Object> responseFields,
	...
	public String getFileStoreId(String tenantId, String module, String filePath)
	...
	private String buildRequestForParentChild(List<List<Object>> filteredList, List<String> columnHeaders,
			Request request, Definition uploadDefinition, DocumentContext documentContext,
			UploaderRequest uploaderRequest, Object previousResponse) throws IOException {
		...
		for (int k = 0; k < filteredList.size(); k++) {
			...
			// this is for setting child objects to empty list in the request if the excel
			// doesn't have values for child.
		...
		if (uploadDefinition.getUniqueKeysForInnerObject() != null) {
			for (String col : uploadDefinition.getUniqueKeysForInnerObject()) {
				StringBuilder expression = new StringBuilder();
				String key = dataUploadUtils.getJsonPathKey(request.getExcelHeadersToRequestMap().get(col).get(0),
						expression);
		...
		try {
			...
			if (!StringUtils.isEmpty(request.getArrayPath())) {
				...
				for (String key : requestMap.keySet()) {
					if (!key.equals("RequestInfo")) {
						oldKey = key;
						List<Object> list = new ArrayList<>();
						list.add(requestMap.get(key));
						value = list;
					}
				}
				if (!StringUtils.isEmpty(oldKey)) {
					requestMap.put(oldKey, value);
				}
			}
		} catch (Exception e) {
			logger.error("Couldn't add Data: ", e);
		}
	...
	private static Map deepMerge(Map original, Map newMap, List<String> uniqueKeysForInnerObject) {
		for (Object key : newMap.keySet()) {
			...
			if (newMap.get(key) instanceof Map && original.get(key) instanceof Map) {
				...
				Map<Object, Object> originalChildEntry = (Map) originalChild.get(originalChild.size() - 1); // other
																											// entries
																											// have been
																											// verified
																											// in
																											// previous
																											// iterations
				Map<Object, Object> newChildEntry = (Map) newChild.get(0); // cuz definitely the list will always have
																			// only one entry.
				int counter = 0;
				try {
					for (String mapKey : uniqueKeysForInnerObject) {
						if (originalChildEntry.get(mapKey).equals(newChildEntry.get(mapKey))) {
							counter++;
						} else {
							break;
						}
	...
	private void writeResultToExcelParentChild(String failureMessage, List<List<Object>> rows,
			List<Object> responseFields, String outputExcelFilepath) throws IOException {
