...
Pageable page = new PageRequest(schemeContractRequest.getPage().getOffSet(),schemeContractRequest.getPage().getPageSize());
		...
		try { 
			...
			case "updateAll": 
				...
				for(SchemeContract b:schemeContractRequest.getSchemes()) 
				 validator.validate(b, errors); 
				break; 
			default : validator.validate(schemeContractRequest.getRequestInfo(), errors); 
			} 
