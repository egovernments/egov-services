...
public class LegacyIndexConsumerConfig implements ApplicationRunner {
	...
	@Value("${egov.default.partions.per.topic}")
	private Integer defaultNoOfPartitions;
	...
	public String setTopics() {
		...
		for (String excludeTopic : excludeArray) {
			if (topicsList.contains(excludeTopic))
				noOfExculdedTopics++;
		}
		String[] topicsArray = new String[topicsList.size() - noOfExculdedTopics];
		int i = 0;
		for (String topic : topicsList) {
			if (Arrays.asList(excludeArray).contains(topic))
				continue;
			topicsArray[i] = topic;
			i++;
		}
	...
	/**
	 * Intializes an array of topics with respective partitions.
	 * 
	 * @return
	 */
	public TopicPartitionInitialOffset[] initializeTopicsAndPartitions() {
		...
		for (int i = 0; i < topicPartitionsInitialOffsetArray.length; i++) {
			TopicPartitionInitialOffset topicPartitionOffset = new TopicPartitionInitialOffset(topics[i],
