...
function getFormTelemetryObject(formKey) {
    ...
    if(formTelemetryObject === undefined) {
        formTelemetryObject = telemetryManager.createFormTelemetry(formKey, window.location.href);
    }
    ...
    if(inputElement) {
        ...
        if(formKey) {
            ...
            if(formTelemetryObject) {
                formTelemetryObject.fieldInteractionEnded(fieldKey, inputElement.value.length);
            }
        }
    }
}, true);
