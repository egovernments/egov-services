...
public class NewUserService {
	...
	public UserRes create(UserReq userReq){
		...
		try{
			userRepository.saveUser(userReq);
		}catch(Exception ex){
			ex.printStackTrace();
			throw ex;
		}
	...
	public UserRes updateAsync(UserReq userReq){
		...
		if(users.size() == dbUsers.size()){
			for(User user : users){
				...
				Map<Long, Address> dbAddressmap = dbAddresses.stream().collect(Collectors.toMap(Address::getId, Function.identity()));
				Map<Long, TenantRole> dbTenantRolemap = dbTenantRoles.stream().collect(Collectors.toMap(TenantRole::getId, Function.identity()));
