...
public class UserQueryBuilderTest {
	...
	private String QueryWithCriteria="select usr.id AS usr_id, usr.tenantid AS usr_tenantid, usr.title,"
			+ " usr.salutation AS usr_salutation, usr.dob AS usr_dob, usr.locale AS usr_locale, usr.username AS usr_username,"
			+ " usr.password AS usr_password, usr.pwdexpirydate AS usr_pwdexpirydate, usr.mobilenumber AS usr_mobilenumber,"
			+ " usr.altcontactnumber AS usr_altcontactnumber, usr.emailid AS usr_emailid, usr.createddate AS usr_createddate,"
			+ " usr.lastmodifieddate AS usr_lastmodifieddate, usr.createdby AS usr_createdby, usr.lastmodifiedby AS usr_lastmodifiedby,"
			+ " usr.active AS usr_active, usr.name AS usr_name, usr.gender AS usr_gender, usr.pan AS usr_pan, usr.aadhaarnumber AS usr_aadhaarnumber,"
			+ " usr.type AS usr_type, usr.version AS usr_version, usr.guardian AS usr_guardian, usr.guardianrelation AS usr_guardianrelation,"
			+ " usr.signature AS usr_signature, usr.accountlocked AS usr_accountlocked, usr.bloodgroup AS usr_bloodgroup,"
			+ " usr.photo AS usr_photo, usr.identificationmark AS usr_identificationmark, address.id AS address_id,"
			+ " address.version AS address_version, address.createddate AS address_createddate, address.lastmodifieddate AS address_lastmodifieddate,"
			+ " address.createdby AS address_createdby, address.lastmodifiedby AS address_lastmodifiedby, address.type AS address_type,"
			+ " address.address AS address_address, address.city AS address_city, address.pincode AS address_pincode, address.userid AS address_userid,"
			+ " address.tenantid AS address_tenantid, role.name AS role_name, role.code AS role_code, role.description AS role_description,"
			+ " role.createddate AS role_createddate, role.createdby AS role_createdby, role.lastmodifiedby AS role_lastmodifiedby, role.lastmodifieddate AS role_lastmodifieddate,"
			+ " role.version AS role_version, role.tenantid AS role_tenantid, role.id AS role_id,  utenanant.id as utenanant_id, utenanant.tenantids as utenanant_tenantids"
			+ " from eg_user usr"
			+ " LEFT OUTER JOIN eg_user_address address ON usr.id = address.userid ANd usr.tenantid = address.tenantid"
			+ " INNER JOIN eg_userrole urole ON usr.id = urole.userid AND usr.tenantid = urole.tenantid"
			+ " INNER JOIN eg_role role ON urole.roleid = role.id AND urole.tenantid = role.tenantid"
			+ " LEFT OUTER JOIN eg_usertenantrole utenanant ON usr.id = utenanant.userid"
			+ " WHERE usr.tenantid = ? AND usr.lastmodifiedby <= ?  AND usr.username IN ('abc','xyz')"
			+ " AND usr.active = ?  AND usr.id IN (1,2) AND usr.type = ?  ORDER BY usr.tenantid ASC,usr_id ASC LIMIT ? OFFSET ?";
	...
	@Test
	public void testGetQueryWithCriteria() {
		List<Object> preparedStatementValues = new ArrayList<>();
		Set<String> orderBy = new HashSet<String>();
		orderBy.add("usr.tenantid");
		orderBy.add("usr_id");
		...
		uNames.add("xyz");
		uNames.add("abc");
		UserSearchCriteria userSearchCriteria=UserSearchCriteria.builder().tenantId("default").id(ids).userName(uNames).active(true)
				.includeDetails(true).lastChangedSince(123l).roleCodes(uNames).type(Type.valueOf("EMPLOYEE")).pageNumber(1).pageSize(2).sort(orderBy).build();
		
		Mockito.doReturn(500).when(userConfigurationUtil).getPageSize();
		
		assertEquals(QueryWithCriteria,
				userQueryBuilder.getQuery(userSearchCriteria, preparedStatementValues));
		...
		expectedPreparedStatementValues.add("EMPLOYEE");
		expectedPreparedStatementValues.add(2);
		expectedPreparedStatementValues.add(0);
		
		assertTrue(preparedStatementValues.toString().equals(expectedPreparedStatementValues.toString()));
		
//		userQueryBuilder.getQuery(UserSearchCriteria.builder().active(null).id(null).includeDetails(null).lastChangedSince(null).roleCodes(null).sort(null).type(null).userName(null).build(), preparedStatementValues);
	}
