...
public class CronService {
	...
	public void fetchData() {
        ...
        VelocityContext context = emailService.formatEmail(email);
	...
	private void enrichBodyWithPGRChannelData(Body body, PGR pgr) {
		...
		for(Map<String, Object> record: data) {
			...
			for(int week = 0; week < noOfWeeks; week++) {
				if(record.get("day").equals(prefix + week)) {
					ivrPerWeek.put("$w" + week+ "pgrchnlivr", record.get("ulbcovered"));
					mobileAppPerWeek.put("$w" + week+ "pgrchnlmapp", record.get("revenuecollected"));
					webAppPerWeek.put("$w" + week+ "pgrchnlweb", record.get("servicesapplied"));
				}
			}
			ivr.add(ivrPerWeek);
