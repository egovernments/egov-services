...
public class ComplaintIndex {
    ...
    public ComplaintIndex() {
        // Default Constructor
    }

    public ComplaintIndex(Map<String, Object> cityMap) {
        registered = 1;
        inProcess = 1;
        closed = false;
        complaintIsClosed = "N";
        durationRange = StringUtils.EMPTY;
        isSLA = "Y";
        ifSLA = 1;
        initialFunctionaryAssigneddate = new Date();
        initialFunctionaryIsSLA = "Y";
        initialFunctionaryIfSLA = 1;
        currentFunctionaryAssigneddate = new Date();
        currentFunctionaryIsSLA = "Y";
        currentFunctionaryIfSLA = 1;
        reasonForRejection = StringUtils.EMPTY;
        cityCode = cityMap.get("cityCode").toString();
        cityDistrictCode = (String) cityMap.get("districtCode");
        cityDistrictName = (String) cityMap.get("districtName");
        cityGrade = (String) cityMap.get("cityGrade");
        cityDomainUrl = (String) cityMap.get("cityurl");
        cityName = (String) cityMap.get("cityname");
        cityRegionName = (String) cityMap.get("cityRegion");
    }
    ...
    public void setCurrentFunctionaryFields() {
        if (Objects.nonNull(currentFunctionaryAssigneddate)) {
            DateTime lastDateToResolve = toDateTime(currentFunctionaryAssigneddate).plusHours(
                    (int) currentFunctionarySLADays);
            if (lastDateToResolve.isAfterNow()) {
                currentFunctionaryAgeingFromDue = 0;
                currentFunctionaryIsSLA = "Y";
                currentFunctionaryIfSLA = 1;
            } else {
                currentFunctionaryAgeingFromDue = Hours.hoursBetween(lastDateToResolve, new DateTime()).getHours();
                currentFunctionaryIsSLA = "N";
                currentFunctionaryIfSLA = 0;
            }
        }
    }

    public void setInitialFunctionaryFields() {
        if (Objects.nonNull(initialFunctionaryAssigneddate)) {
            DateTime lastDateToResolve = toDateTime(initialFunctionaryAssigneddate).plusHours(
                    (int) initialFunctionarySLADays);
            if (lastDateToResolve.isAfterNow()) {
                initialFunctionaryAgeingFromDue = 0;
                initialFunctionaryIsSLA = "Y";
                initialFunctionaryIfSLA = 1;
            } else {
                initialFunctionaryAgeingFromDue = Hours.hoursBetween(lastDateToResolve, new DateTime()).getHours();
    ...
    public void setComplaintFields() {
        final DateTime currentDate = new DateTime();
        complaintPeriod = Days.daysBetween(toDateTime(createdDate), currentDate).getDays();
        DateTime lastDateToResolve = toDateTime(createdDate).plusHours(
                (int) complaintSLADays);
        if (lastDateToResolve.isAfterNow()) {
            complaintAgeingFromDue = 0;
            complaintAgeingdaysFromDue = 0;
            isSLA = "Y";
            ifSLA = 1;
        }
        else{
            complaintAgeingFromDue = Hours.hoursBetween(lastDateToResolve, currentDate).getHours();
            complaintAgeingdaysFromDue = Days.daysBetween(lastDateToResolve, currentDate).getDays();
            isSLA = "N";
            ifSLA = 0;
        }
    }

    public void setFirstLevelEscalationFields() {
        if (Objects.nonNull(escalation1FunctionaryAssigneddate)) {
            DateTime lastDateToResolve = toDateTime(escalation1FunctionaryAssigneddate).plusHours(
                    (int) escalation1FunctionarySLADays);
            if (lastDateToResolve.isAfterNow()) {
                escalation1FunctionaryAgeingFromDue = 0;
                escalation1FunctionaryIsSLA = "Y";
                escalation1FunctionaryIfSLA = 1;
            } else {
                escalation1FunctionaryAgeingFromDue = Hours.hoursBetween(lastDateToResolve, new DateTime()).getHours();
                escalation1FunctionaryIsSLA = "N";
                escalation1FunctionaryIfSLA = 0;
            }
        }
    }

    public void setSecondLevelEscalationFields() {
        if (Objects.nonNull(escalation2FunctionaryAssigneddate)) {
            DateTime lastDateToResolve = toDateTime(escalation2FunctionaryAssigneddate).plusHours(
                    (int) escalation2FunctionarySLADays);
            if (lastDateToResolve.isAfterNow()) {
                escalation2FunctionaryAgeingFromDue = 0;
                escalation2FunctionaryIsSLA = "Y";
                escalation2FunctionaryIfSLA = 1;
            } else {
                escalation2FunctionaryAgeingFromDue = Hours.hoursBetween(lastDateToResolve, new DateTime()).getHours();
                escalation2FunctionaryIsSLA = "N";
                escalation2FunctionaryIfSLA = 0;
            }
        }
    }

    public void setThirdLevelEscalationFields() {
        if (Objects.nonNull(escalation3FunctionaryAssigneddate)) {
            DateTime lastDateToResolve = toDateTime(escalation3FunctionaryAssigneddate).plusHours(
                    (int) escalation3FunctionarySLADays);
            if (lastDateToResolve.isAfterNow()) {
                escalation3FunctionaryAgeingFromDue = 0;
                escalation3FunctionaryIsSLA = "Y";
                escalation3FunctionaryIfSLA = 1;
            } else {
                escalation3FunctionaryAgeingFromDue = Hours.hoursBetween(lastDateToResolve, new DateTime()).getHours();
                escalation3FunctionaryIsSLA = "N";
                escalation3FunctionaryIfSLA = 0;
            }
        }
    }

    // Method to convert Date to Joda DateTime
