...
public class ComplaintIndexRepositoryImpl implements ComplaintIndexCustomRepository {
    ...
    public SearchResponse findAllGrievanceByComplaintType(ComplaintDashBoardRequest complaintDashBoardRequest,
    	...
    	SearchQuery search = new NativeSearchQueryBuilder().withQuery(query).build();
...
//        
	...
	@Override
	public SearchResponse findAllFromUlb() {
//		SearchResponse response = elasticsearchTemplate.getClient().prepareSearch(PGR_INDEX_NAME)
//				.addAggregation(AggregationBuilders.terms("ulblevel").field("cityCode").size(120)
//						        .subAggregation(AggregationBuilders.terms("depatrtmentLevel").field("departmentCode").size(120)
//						        		.subAggregation(AggregationBuilders.terms("functionarylevel").field("currentFunctionaryName").size(120)
//						        				.subAggregation(AggregationBuilders.topHits("top").setSize(1)))))
//				.execute().actionGet();
		
		SearchResponse response = elasticsearchTemplate.getClient().prepareSearch(PGR_INDEX_NAME).setSize(0)
				.addAggregation(AggregationBuilders.terms("ulbwise").field("cityCode").size(120)
						.subAggregation(AggregationBuilders.terms("departmentwise").field("departmentCode").size(1000)
						                .subAggregation(AggregationBuilders.terms("functionarywise").field("currentFunctionaryName").size(1000)
