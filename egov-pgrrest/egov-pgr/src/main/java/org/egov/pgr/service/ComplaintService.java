...
public class ComplaintService {
	...
	public Complaint createComplaint(final Complaint complaint) throws ValidationException {
		...
		complaint.transition().start().withSenderName(complaint.getComplainant().getName())
				...
				.withStateValue(complaint.getStatus().getName()).withOwner(complaint.getAssignee())
		...
		else if (null != complaint.getAssignee())
		...
		// Commenting the ElasticSEarch part
	...
	public Complaint update(final Complaint complaint, final Long approvalPosition, final String approvalComent) {
		...
		// if
		// (securityUtils.getCurrentUser().getType().equals(UserType.CITIZEN))
		// userName = securityUtils.getCurrentUser().getName();
		// else
		// userName = securityUtils.getCurrentUser().getUsername() +
		// DELIMITER_COLON + securityUtils.getCurrentUser().getName();
	...
	private String getHeaderMessage(final Complaint savedComplaint) {
		...
		if (savedComplaint.getStatus() != null && savedComplaint.getStatus().getName() != null
				&& savedComplaint.getStatus().getName().equals("REGISTERED"))
	...
	private String getDetailedMessage(final Complaint savedComplaint) {
		...
		detailedMessage.append("Grievance No. ").append(savedComplaint.getCrn()).append(" regarding ")
				...
				.append(savedComplaint.getStatus().getName() != null ? savedComplaint.getStatus().getName() : "")
				.append(" status.");
	...
	public List<Hashtable<String, Object>> getHistory(final Complaint complaint) {
		...
		if (!complaint.getStateHistory().isEmpty() && complaint.getStateHistory() != null) {
			...
			for (final StateHistory stateHistory : complaintStateHistory) {
				...
				HistoryMap.put(COMMENT,
						stateHistory.getComments() != null ? stateHistory.getComments() : StringUtils.EMPTY);
				...
				if (null != user) {
					...
					final List<Assignment> assignments = assignmentService.getAssignmentsForPosition(owner.getId(),
							new Date());
			...
			if (!assignments.isEmpty()) {
				...
				if (null != user) {
					final StringBuffer smsBodyOfficial = new StringBuffer().append("New Grievance for ")
							.append(complaint.getComplaintType().getName()).append(" is registered by ")
					...
					if (complaint.getLatlngAddress() != null)
						...
						smsBodyOfficial.append(complaint.getChildLocation() != null
								? ", " + complaint.getChildLocation().getName() : StringUtils.EMPTY);
					smsBodyOfficial.append(complaint.getLandmarkDetails() != null
							? ", " + complaint.getLandmarkDetails() : StringUtils.EMPTY);
	...
	public void sendSmsOnCompletion(final Complaint complaint) {
		final StringBuffer smsBody = new StringBuffer().append("Your Grievance regarding ")
				.append(complaint.getComplaintType().getName()).append(" with tracking number '")
				.append(complaint.getCrn()).append("' is updated to ").append(complaint.getStatus().getName());
	...
	public void sendSmsToOfficials(final Complaint complaint) {
		...
		senderName = complaint.getState().getSenderName().contains(DELIMITER_COLON)
				? complaint.getState().getSenderName().split(DELIMITER_COLON)[1] : complaint.getState().getSenderName();
		if (null != owner && null != owner.getDeptDesig()) {
	...
	public String getEmailBody(final Complaint complaint) {
		final String formattedCreatedDate = new SimpleDateFormat("dd/MM/yyyy HH:mm").format(complaint.getCreatedDate());
		final StringBuffer emailBody = new StringBuffer().append(" %0D%0A Grievance Details -  %0D%0A %0D%0A CRN - ")
				.append(complaint.getCrn()).append(" %0D%0A Grievance Type -")
				.append(complaint.getComplaintType().getName());
		...
		emailBody.append(" %0D%0A Grievance details - ").append(complaint.getDetails())
				.append(" %0D%0A Grievance status -").append(complaint.getStatus().getName())
				.append(" %0D%0A Grievance Registration Date - ").append(formattedCreatedDate);
	...
	public List<Complaint> getPendingGrievances() {
		...
		criteria.add(
				Restrictions.eq("complaint.assignee", positionMasterService.getCurrentPositionForUser(user.getId())));
		return criteria.list();
	}
	...
	public HashMap<String, Integer> getMyComplaintsCount() {
		...
		complaintsCount.put(COMPLAINT_PENDING, complaintRepository
				.getMyComplaintCountByStatus(securityUtils.getCurrentUser(), pendingStatus).intValue());
		complaintsCount.put(COMPLAINT_COMPLETED, complaintRepository
				.getMyComplaintCountByStatus(securityUtils.getCurrentUser(), completedStatus).intValue());
		complaintsCount.put(COMPLAINT_REJECTED, complaintRepository
				.getMyComplaintCountByStatus(securityUtils.getCurrentUser(), rejectedStatus).intValue());
	...
	public HashMap<String, Integer> getComplaintsTotalCount() {
		...
		complaintsCount.put(COMPLAINTS_RESOLVED,
				complaintRepository.getComplaintsTotalCountByStatus(resolvedStatus).intValue());
		complaintsCount.put(COMPLAINTS_UNRESOLVED,
				complaintRepository.getComplaintsTotalCountByStatus(pendingStatus).intValue());
		return complaintsCount;
	}
	...
	public List<Complaint> getBySearchInputs(String code, String status, String crn, String start_date,
			String creater_id) throws ParseException {

		final Criteria criteria = entityManager.unwrap(Session.class).createCriteria(Complaint.class, "complaint")
				.createAlias("complaint.status", "status").createAlias("complaint.complaintType", "complainttype")
				.createAlias("complaint.createdBy", "createdBy");
		...
		if (StringUtils.isNotBlank(creater_id))
			criteria.add(Restrictions.eq("createdBy.id", Long.valueOf(creater_id)));

		return criteria.list();
	}

	private void sendMessage(Complaint complaint, StringBuffer emailSubject) throws JsonProcessingException {
		...
		emailRequest.setSubject("Grievance Registered Successfully");
		emailRequest.setRequestInfo(requestInfo);
		ObjectMapper mapper = new ObjectMapper();

		SMSRequest smsRequest = new SMSRequest();
		smsRequest.setRequestInfo(requestInfo);
		smsRequest.setMobile_no(complaint.getComplainant().getMobile());
		...
		producer.flush();
		producer.send(new ProducerRecord<String, String>("egov-notification-email", "pgrrest",
