...
public class ComplaintRouterService {
    ...
    public PositionResponse getAssignee(final Long boundaryId, final String complaintTypeCode, final Long assigneeId) {
        ...
        if (assigneeId == null) {
                ...
                if (StringUtils.isNotBlank(complaintTypeCode)) {
                    for (final Boundary bndry : boundaries) {
                        complaintRouter = complaintRouterRepository
                                .findByComplaintType_CodeAndBoundary_Id(complaintTypeCode, bndry.getId());
                        if (null != complaintRouter)
                            break;
                    }
                    if (null == complaintRouter)
                        complaintRouter = complaintRouterRepository.findByOnlyComplaintType_Code(complaintTypeCode);
                    if (null == complaintRouter)
                        for (final Boundary bndry : boundaries) {
            ...
            if (complaintRouter != null) {
                positions = getPositionById(complaintRouter.getPosition().getId());
                if (!positions.isEmpty())
                    positionResponse = positions.iterator().next();
            }
            ...
            try {
                positionHierarchies = getAssigneePosition("Complaint", assigneeId, complaintTypeCode);
                if (positionHierarchies.isEmpty() || positionHierarchies.contains(null)) {
                    final List<EmployeeResponse> employees = getUsersByRoleName("Grievance Routing Officer");
                    if (!employees.isEmpty())
                        employeeResponse = employees.iterator().next();
                    if (employeeResponse != null)
                        positions = getPositionsForEmployee(employeeResponse.getCode());
                    if (!positions.isEmpty())
                        positionResponse = getPositionById(positions.iterator().next().getId()).iterator().next();
                } else
    ...
    private List<PositionHierarchyResponse> getAssigneePosition(final String objectType, final Long currentAssigneeId,
            final String objectSubType) {
        // TODO - Externalize the url to the user details
        final String url = String.format("http://localhost:8080/eis/positionhierarchies?tenantId=ap.public&objectType="
                + objectType + "&fromPosition=" + currentAssigneeId + "&objectSubType=" + objectSubType);
        return new RestTemplate().getForObject(url, PositionsResponse.class).getPositionHierarchies();
    }

    private List<EmployeeResponse> getUsersByRoleName(final String roleName) {
        final String url = String.format("http://localhost:8080/eis/employee?tenantId=ap.public&roleName=" + roleName);
        return new RestTemplate().getForObject(url, EmployeesResponse.class).getEmployees();
    }

    private List<PositionResponse> getPositionsForEmployee(final String code) {
        final String url = String.format("http://localhost:8080/eis/employee/" + code + "/positions?tenantId=ap.public");
        return new RestTemplate().getForObject(url, PositionsResponse.class).getPositions();
    }

    private List<PositionResponse> getPositionById(final Long positionId) {
        final String url = String.format("http://localhost:8080/eis/position?tenantId=ap.public&id=" + positionId);
        return new RestTemplate().getForObject(url, PositionsResponse.class).getPositions();
    }
    ...
    public Boolean validateRouter(final ComplaintRouter complaintRouter) {
        ...
        if (null != complaintRouter.getComplaintType() && null != complaintRouter.getBoundary())
            queryResult = complaintRouterRepository.findByComplaintType_CodeAndBoundary_Id(
                    complaintRouter.getComplaintType().getCode(), complaintRouter.getBoundary().getId());
        ...
        if (null != complaintRouter.getComplaintType() && null == complaintRouter.getBoundary())
            queryResult = complaintRouterRepository
                    .findByOnlyComplaintType_Code(complaintRouter.getComplaintType().getCode());
        if (queryResult != null)
            exist = true;
        return exist;
    }
    ...
    public ComplaintRouter getExistingRouter(final ComplaintRouter complaintRouter) {
        ...
        if (null != complaintRouter.getComplaintType() && null != complaintRouter.getBoundary())
            router = complaintRouterRepository.findByComplaintType_CodeAndBoundary_Id(
                    complaintRouter.getComplaintType().getCode(), complaintRouter.getBoundary().getId());
        if (null != complaintRouter.getBoundary() && null == complaintRouter.getComplaintType())
            router = complaintRouterRepository.findByOnlyBoundary(complaintRouter.getBoundary());
        if (null != complaintRouter.getComplaintType() && null == complaintRouter.getBoundary())
            router = complaintRouterRepository
                    .findByOnlyComplaintType_Code(complaintRouter.getComplaintType().getCode());
