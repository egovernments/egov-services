...
private SubSchemeContractRepository subSchemeContractRepository;
	...
	private BindingResult validate(List<BillRegister> billregisters, String method, BindingResult errors) {
                ...
                try {
                    switch (method) {
                    case ACTION_VIEW:
                        // validator.validate(billRegisterContractRequest.getBillRegister(), errors);
                        break;
                    case Constants.ACTION_CREATE:
                        ...
                        for (BillRegister billRegister : billregisters) {
                            ...
                            if (!billRegisterRepository.uniqueCheck("name", billRegister)) {
                                errors.addError(new FieldError("billRegister", "name", billRegister.getName(), false,
                                        new String[] { ErrorCode.NON_UNIQUE_VALUE.getCode() }, null, null));
                            }
                            if (!billRegisterRepository.uniqueCheck("code", billRegister)) {
                                errors.addError(new FieldError("billRegister", "code", billRegister.getName(), false,
                                        new String[] { ErrorCode.NON_UNIQUE_VALUE.getCode() }, null, null));
                            }
                            if (!billRegisterRepository.uniqueCheck("identifier", billRegister)) {
                                errors.addError(new FieldError("billRegister", "identifier", billRegister.getName(), false,
                                        new String[] { ErrorCode.NON_UNIQUE_VALUE.getCode() }, null, null));
                            }
                    ...
                    case Constants.ACTION_UPDATE:
                        ...
                        for (BillRegister billRegister : billregisters) {
                            if (billRegister.getId() == null) {
                                throw new InvalidDataException("id", ErrorCode.MANDATORY_VALUE_MISSING.getCode(), billRegister.getId());
                            }
                            validator.validate(billRegister, errors);
                            if (!billRegisterRepository.uniqueCheck("name", billRegister)) {
                                errors.addError(new FieldError("billRegister", "name", billRegister.getName(), false,
