...
public class ChecklistJdbcRepository extends JdbcRepository {
    private static final Logger LOG = LoggerFactory.getLogger(ChecklistJdbcRepository.class);
    static {
    ...
    public ChecklistJdbcRepository(NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
	...
	if (checklistSearchEntity.getSortBy() != null && !checklistSearchEntity.getSortBy().isEmpty()) {
	    validateSortByOrder(checklistSearchEntity.getSortBy());
	    validateEntityFieldName(checklistSearchEntity.getSortBy(), ChecklistEntity.class);
	}
	...
	if (checklistSearchEntity.getDescription() != null) {
	    ...
	    paramValues.put("description", checklistSearchEntity.getDescription());
	}
	...
	List<ChecklistEntity> checklistEntities = namedParameterJdbcTemplate.query(searchQuery.toString(), paramValues,
		row);
	...
	List<ChecklistEntity> checklists = namedParameterJdbcTemplate.query(
		...
		new BeanPropertyRowMapper(ChecklistEntity.class));
