...
public abstract class ESRepository {
    ...
    public void validateSortByOrder(final String sortBy) {
        ...
        for (String s : sortByList) {
            if (s.contains(" ")
                ...
                customException.setCode(s.split(" ")[0]);
                customException
                        .setMessage("Please send the proper sortBy order for the field " + s.split(" ")[0]);
    ...
    public void validateEntityFieldName(String sortBy, final Class<?> object) {
        ...
        for (String s : sortByList) {
            ...
            if (!isFieldExist) {
                customException.setCode(s.contains(" ") ? s.split(" ")[0] : s);
                customException.setMessage("Please send the proper Field Names ");
                throw customException;
            }
        }
