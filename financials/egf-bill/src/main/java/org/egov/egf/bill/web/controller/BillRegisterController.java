...
public class BillRegisterController {
	...
	billregisters = billRegisterService.create(billregisters, errors, billRegisterRequest.getRequestInfo());
    ...
    @PostMapping("/_update")
    @ResponseStatus(HttpStatus.CREATED)
    public BillRegisterResponse update(@RequestBody BillRegisterRequest billRegisterRequest, BindingResult errors) {
	if (errors.hasErrors()) {
	    throw new CustomBindException(errors);
	}
	...
	billregisters = billRegisterService.update(billregisters, errors, billRegisterRequest.getRequestInfo());
	...
	ModelMapper model = new ModelMapper();
	List<BillRegisterContract> billRegisterContracts = new ArrayList<>();
	Pagination<BillRegister> billregisters = billRegisterService.search(domain, errors);
