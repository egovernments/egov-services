...
public class InstrumentTypeMapperTest {
	...
	@Before
	public void setup() {
		instrumentTypeMapper = new InstrumentTypeMapper();
	}

	@Test
	public void test_to_domain() {

		InstrumentType expectedDomain = instrumentTypeMapper.toDomain(contract());

		assertEquals(expectedDomain.getId(), domain().getId());
		assertEquals(expectedDomain.getActive(), domain().getActive());
		assertEquals(expectedDomain.getDescription(), domain().getDescription());
		assertEquals(expectedDomain.getName(), domain().getName());
		assertEquals(expectedDomain.getInstrumentTypeProperties().get(0).getId(),
		...
		assertEquals(expectedDomain.getInstrumentTypeProperties().get(0).getTransactionType(),
				domain().getInstrumentTypeProperties().get(0).getTransactionType());
	...
	public void test_to_contract() {
		...
		assertEquals(expectedContract.getInstrumentTypeProperties().get(0).getId(),
	...
	public void test_to_search_domain() {
		...
		InstrumentTypeSearch expectedSearchDomain = instrumentTypeMapper.toSearchDomain(searchContract());

		assertEquals(expectedSearchDomain.getId(), searchDomain().getId());
		assertEquals(expectedSearchDomain.getActive(), searchDomain().getActive());
	...
	public InstrumentType domain() {
		...
		instrumentType.setCreatedBy(User.builder().id(1l).build());
		instrumentType.setLastModifiedBy(User.builder().id(1l).build());
		instrumentType.setTenantId("tenantId");

		return instrumentType;
	}

	public InstrumentTypeContract contract() {
	...
	public InstrumentTypeSearch searchDomain() {
		...
		instrumentTypeSearch.setLastModifiedBy(User.builder().id(1l).build());
		instrumentTypeSearch.setTenantId("tenantId");
		instrumentTypeSearch.setPageSize(1);
		instrumentTypeSearch.setOffset(1);

		return instrumentTypeSearch;
	}

	public InstrumentTypeSearchContract searchContract() {
