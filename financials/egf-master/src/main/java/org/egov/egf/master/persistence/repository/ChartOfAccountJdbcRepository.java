...
paramValues.put("isSubLedger" ,chartOfAccountSearchEntity.getIsSubLedger());} 
		...
		searchQuery = searchQuery.replace(":pagination", "limit " + chartOfAccountSearchEntity.getPageSize() + " offset "
				+ chartOfAccountSearchEntity.getOffset() * chartOfAccountSearchEntity.getPageSize());

		BeanPropertyRowMapper row = new BeanPropertyRowMapper(ChartOfAccountEntity.class);

		List<ChartOfAccountEntity> chartOfAccountEntities = namedParameterJdbcTemplate.query(searchQuery.toString(), paramValues, row);
	...
	public ChartOfAccountEntity findById(ChartOfAccountEntity entity) {
		...
		List<ChartOfAccountEntity> chartofaccounts = jdbcTemplate.query(getByIdQuery.get(entity.getClass().getSimpleName()),
				preparedStatementValues.toArray(), new BeanPropertyRowMapper<ChartOfAccountEntity>());
