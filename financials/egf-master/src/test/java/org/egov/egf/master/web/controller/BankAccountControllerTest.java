...
public class BankAccountControllerTest {
    ...
    @Captor
    private ArgumentCaptor<BankAccountRequest> captor;
    ...
    public void testCreate() throws IOException, Exception {
        when(bankAccountService.add(any(List.class), any(BindingResult.class))).thenReturn((getBankAccounts()));

        mockMvc.perform(
                post("/bankaccounts/_create").content(resources.readRequest("bankaccount/bankAccount_create_valid_request.json"))
                        .contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().is(201))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(content().json(resources.readResponse("bankaccount/bankAccount_create_valid_response.json")));

        verify(bankAccountService).addToQue(captor.capture());

        final BankAccountRequest actualRequest = captor.getValue();
        assertEquals("1", actualRequest.getBankAccounts().get(0).getBankBranch().getId());
        assertEquals("1", actualRequest.getBankAccounts().get(0).getChartOfAccount().getId());
        assertEquals("2", actualRequest.getBankAccounts().get(0).getFund().getId());
        assertEquals("0004", actualRequest.getBankAccounts().get(0).getAccountNumber());
        assertEquals(true, actualRequest.getBankAccounts().get(0).getActive());
    }

    @Test
    public void testUpdate() throws IOException, Exception {
        when(bankAccountService.update(any(List.class), any(BindingResult.class))).thenReturn((getUpdatedBankAccounts()));

        mockMvc.perform(
                post("/bankaccounts/_update").content(resources.readRequest("bankaccount/bankAccount_update_valid_request.json"))
                        .contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().is(201))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(content().json(resources.readResponse("bankaccount/bankAccount_update_valid_response.json")));
        ...
        assertEquals("2", actualRequest.getBankAccounts().get(0).getFund().getId());
        assertEquals("0004", actualRequest.getBankAccounts().get(0).getAccountNumber());
        assertEquals("testupdate", actualRequest.getBankAccounts().get(0).getAccountType());
        assertEquals(true, actualRequest.getBankAccounts().get(0).getActive());
    }
    ...
    public void testSearch() throws IOException, Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(content().json(resources.readResponse("bankaccount/bankAccount_search_valid_response.json")));
    }

    private List<BankAccount> getBankAccounts() {
        List<BankAccount> bankAccounts = new ArrayList<BankAccount>();
        BankAccount bankAccount = BankAccount.builder().accountNumber("0004").accountType("test").description("accountNumber")
                .active(true).payTo("tester").build();
