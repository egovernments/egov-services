...
public class BankControllerTest {
    ...
    @Captor
    private ArgumentCaptor<BankRequest> captor;
    ...
    public void testCreate() throws IOException, Exception {
        ...
        verify(bankService).addToQue(captor.capture());

        final BankRequest actualRequest = captor.getValue();
        assertEquals("name", actualRequest.getBanks().get(0).getName());
        assertEquals("code", actualRequest.getBanks().get(0).getCode());
        assertEquals("default", actualRequest.getBanks().get(0).getTenantId());
        assertEquals("description", actualRequest.getBanks().get(0).getDescription());
        assertEquals(true, actualRequest.getBanks().get(0).getActive());
    }

    @Test
    public void testUpdate() throws IOException, Exception {
        when(bankService.update(any(List.class), any(BindingResult.class))).thenReturn((getUpdateBanks()));
        ...
        final BankRequest actualRequest = captor.getValue();
        assertEquals("nameupdate", actualRequest.getBanks().get(0).getName());
        assertEquals("codeupdate", actualRequest.getBanks().get(0).getCode());
        assertEquals("descriptionupdate", actualRequest.getBanks().get(0).getDescription());
        assertEquals(true, actualRequest.getBanks().get(0).getActive());
        assertEquals("default", actualRequest.getBanks().get(0).getTenantId());
    }
    ...
    public void testCreate_Error() throws IOException, Exception {
            ...
            mockMvc.perform(post("/banks/_create").content(resources.readRequest("bank/bank_create_invalid_field_value.json"))
    ...
    private List<Bank> getBanks() {
        ...
        Bank bank = Bank.builder().code("code").name("name").description("description").active(true).type("type").build();
