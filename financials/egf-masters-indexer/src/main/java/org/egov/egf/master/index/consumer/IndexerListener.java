...
public class IndexerListener {
    ...
    @KafkaListener(id = "${kafka.topics.egov.index.id}", topics = { "${kafka.topics.egov.index.name}",
            "${kafka.topics.egf.budget.completed.topic}",
            "${kafka.topics.egf.instrument.completed.topic}" }, group = "${kafka.topics.egov.index.group}")
    public void listen(final HashMap<String, Object> financialContractRequestMap) {
        ...
        if (financialContractRequestMap.get("budget_persisted") != null) {
            ...
            final FinancialStatusRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("budget_persisted"),
                    FinancialStatusRequest.class);
        ...
        if (financialContractRequestMap.get("budgetreappropriation_persisted") != null) {

            final BudgetReAppropriationRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("budgetreappropriation_persisted"),
                    BudgetReAppropriationRequest.class);
            ...
            if (request.getBudgetReAppropriations() != null && !request.getBudgetReAppropriations().isEmpty())
                for (final BudgetReAppropriationContract budgetReAppropriationContract : request.getBudgetReAppropriations()) {
                    ...
                    elasticSearchRepository.index(BUDGETREAPPROPRIATION_OBJECT_TYPE,
        ...
        if (financialContractRequestMap.get("budgetdetail_persisted") != null) {

            final BudgetDetailRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("budgetdetail_persisted"),
                    BudgetDetailRequest.class);
            ...
            if (request.getBudgetDetails() != null && !request.getBudgetDetails().isEmpty())
                for (final BudgetDetailContract budgetDetailContract : request.getBudgetDetails()) {
                    ...
                    elasticSearchRepository.index(BUDGETDETAIL_OBJECT_TYPE,
                            budgetDetailContract.getTenantId() + "-" + budgetDetailContract.getBudget().getName(), indexObj);
                }
        }

        if (financialContractRequestMap.get("budget_persisted") != null) {

            final BudgetRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("budget_persisted"),
                    BudgetRequest.class);

            if (request.getBudgets() != null && !request.getBudgets().isEmpty())
                for (final BudgetContract budgetContract : request.getBudgets()) {
                    final HashMap<String, Object> indexObj = getBudgetContractIndexObject(budgetContract);
                    elasticSearchRepository.index(BUDGET_OBJECT_TYPE,
                            budgetContract.getTenantId() + "-" + budgetContract.getName(), indexObj);
                }
        }

        if (financialContractRequestMap.get("instrumentaccountcode_persisted") != null) {

            final InstrumentAccountCodeRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("instrumentaccountcode_persisted"),
                    InstrumentAccountCodeRequest.class);

            if (request.getInstrumentAccountCodes() != null && !request.getInstrumentAccountCodes().isEmpty())
                for (final InstrumentAccountCodeContract instrumentAccountCodeContract : request.getInstrumentAccountCodes()) {
        ...
        if (financialContractRequestMap.get("instrument_persisted") != null) {

            final InstrumentRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("instrument_persisted"),
            ...
            if (request.getInstruments() != null && !request.getInstruments().isEmpty())
                for (final InstrumentContract instrumentContract : request.getInstruments()) {
                    ...
                    elasticSearchRepository.index(INSTRUMENT_OBJECT_TYPE,
                            instrumentContract.getTenantId() + "-" + instrumentContract.getTransactionNumber(), indexObj);
                }
        }

        if (financialContractRequestMap.get("surrenderreason_persisted") != null) {

            final SurrenderReasonRequest request = objectMapper.convertValue(
                    financialContractRequestMap.get("surrenderreason_persisted"),
            ...
            if (request.getSurrenderReasons() != null && !request.getSurrenderReasons().isEmpty())
                for (final SurrenderReasonContract surrenderReasonContract : request.getSurrenderReasons()) {
                    final HashMap<String, Object> indexObj = getSurrenderReasonContractIndexObject(surrenderReasonContract);
    ...
    private HashMap<String, Object> getSurrenderReasonContractIndexObject(final SurrenderReasonContract surrenderReasonContract) {
    ...
    private HashMap<String, Object> getBudgetDetailContractIndexObject(final BudgetDetailContract budgetDetailContract) {
