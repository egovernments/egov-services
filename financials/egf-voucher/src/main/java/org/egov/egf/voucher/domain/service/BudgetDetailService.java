...
import org.egov.egf.voucher.domain.repository.BudgetDetailRepository;
...
private BoundaryContractRepository boundaryContractRepository;
	...
	private BindingResult validate(List<BudgetDetail> budgetdetails, String method, BindingResult errors) {
		...
		try {
			switch (method) {
			case ACTION_VIEW:
				// validator.validate(budgetDetailContractRequest.getBudgetDetail(), errors);
				break;
			case ACTION_CREATE:
...
{
DepartmentContract  usingDepartment=departmentContractRepository.findById(budgetDetail.getUsingDepartment());
if(usingDepartment==null)
{
throw new InvalidDataException("usingDepartment","usingDepartment.invalid"," Invalid usingDepartment");
}budgetDetail.setUsingDepartment(usingDepartment);
}
if(budgetDetail.getExecutingDepartment()!=null)
{
DepartmentContract  executingDepartment=departmentContractRepository.findById(budgetDetail.getExecutingDepartment());
if(executingDepartment==null)
{
throw new InvalidDataException("executingDepartment","executingDepartment.invalid"," Invalid executingDepartment");
...
throw new InvalidDataException("status","status.invalid"," Invalid status");
	...
	@Transactional
	public List<BudgetDetail> add(List<BudgetDetail> budgetdetails, BindingResult errors) {
	...
	public void addToQue(CommonRequest<BudgetDetailContract> request) {
		budgetDetailRepository.add(request);
	}
