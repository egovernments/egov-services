...
/*	public Pagination<LedgerDetail> search(LedgerDetailSearch domain) {
		LedgerDetailSearchEntity ledgerDetailSearchEntity = new LedgerDetailSearchEntity();
		...
		if (ledgerDetailSearchEntity.getSortBy() != null && !ledgerDetailSearchEntity.getSortBy().isEmpty()) {
			validateSortByOrder(ledgerDetailSearchEntity.getSortBy());
			validateEntityFieldName(ledgerDetailSearchEntity.getSortBy(), LedgerDetailEntity.class);
		}

		String orderBy = "order by name";
		if (ledgerDetailSearchEntity.getSortBy() != null && !ledgerDetailSearchEntity.getSortBy().isEmpty()) {
			orderBy = "order by " + ledgerDetailSearchEntity.getSortBy();
		}
		...
		if (ledgerDetailSearchEntity.getIds() != null) {
			if (params.length() > 0)
				params.append(" and ");
		...
		if (ledgerDetailSearchEntity.getCode() != null) {
			if (params.length() > 0) {
				params.append(" and ");
			}
			params.append("code =:code");
			paramValues.put("code", ledgerDetailSearchEntity.getCode());
		}
		if (ledgerDetailSearchEntity.getName() != null) {
			if (params.length() > 0) {
				params.append(" and ");
			}
			params.append("name =:name");
			paramValues.put("name", ledgerDetailSearchEntity.getName());
		}
		if (ledgerDetailSearchEntity.getIdentifier() != null) {
			if (params.length() > 0) {
				params.append(" and ");
			}
			params.append("description =:description");
			paramValues.put("description", ledgerDetailSearchEntity.getIdentifier());
		}
		if (ledgerDetailSearchEntity.getActive() != null) {
			if (params.length() > 0) {
				params.append(" and ");
			}
			params.append("active =:active");
			paramValues.put("active", ledgerDetailSearchEntity.getActive());
		}
		if (ledgerDetailSearchEntity.getLevel() != null) {
			if (params.length() > 0) {
				params.append(" and ");
			}
		...
		if (ledgerDetailSearchEntity.getOffset() != null) {
			page.setOffset(ledgerDetailSearchEntity.getOffset());
		}
		if (ledgerDetailSearchEntity.getPageSize() != null) {
		...
		List<LedgerDetailEntity> ledgerDetailEntities = namedParameterJdbcTemplate.query(searchQuery.toString(),
				paramValues, row);

		page.setTotalResults(ledgerDetailEntities.size());

		List<LedgerDetail> ledgerdetails = new ArrayList<>();
