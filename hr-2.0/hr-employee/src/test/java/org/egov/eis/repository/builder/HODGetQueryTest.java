...
public class HODGetQueryTest {
	...
	public HODGetQueryTest(HODEmployeeCriteria criteria, String expectedQuery, Map<String, Object> expectedParams) {
	...
	public static Collection testConditions() {
		Object[][] conditions = {{
				...
				HODEmployeeCriteria.builder().tenantId("default").sortBy("e.code").build(),
				expectedQueries[3],
				new HashMap<String, Object>() {{
					put("tenantId", "default");
					put("ids", Arrays.asList(1L, 2L));
				...
				HODEmployeeCriteria.builder().tenantId("default").sortOrder("DESC").build(),
				expectedQueries[4],
				new HashMap<String, Object>() {{
					put("tenantId", "default");
					put("ids", Arrays.asList(1L, 2L));
	...
	public void testGetQuery() {
		...
		assertEquals(expectedQuery, hodEmployeeQueryBuilder.getQuery(criteria, namedParameters, Arrays.asList(1L, 2L)));
	...
	private static String[] getFileContents() {
		...
		try {
			expectedQueries = new FileUtils()
					.getFileContents("org/egov/eis/repository/builder/HODGetQueriesContainer.txt")
                    .split("\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
