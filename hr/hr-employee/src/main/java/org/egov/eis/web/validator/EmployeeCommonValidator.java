...
*/
    ...
    public void validatePrimaryPositions(List<Assignment> assignments, Long employeeId, String tenantId, Errors errors,
                                         String requestType) {
        for(int i = 0; i < assignments.size(); i++) {
            if (assignments.get(i).getIsPrimary()) {
                if (nonVacantPositionsRepository.checkIfPositionIsOccupied(assignments.get(i), employeeId, tenantId, requestType)) {
                    errors.rejectValue("employee.assignments[" + i + "].position", "redundant", "primary position(s) concurrent");
                }
            }
        }
    }
    ...
    protected List<Integer> getIndex(Employee employee, EntityType entityType, String document) {
        switch (entityType) {
            case EMPLOYEE_HEADER:
                for (int i = 0; i < employee.getDocuments().size(); i++)
