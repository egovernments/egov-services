...
public class EmployeeQueryBuilderTest {
	...
	public void testEmptyQuery() {
		...
		EmployeeCriteria employeeCriteria = new EmployeeCriteria().builder().tenantId("1").build();
		List<Object> preparedStatementValues = new ArrayList<>();

		String queryString = employeeQueryBuilder.getQueryForListOfEmployeeIds(employeeCriteria, preparedStatementValues);
		String expectedQueryString = "SELECT distinct e.id AS id  FROM egeis_employee e JOIN egeis_assignment a ON e.id = a.employeeId LEFT JOIN egeis_hodDepartment hod ON a.id = hod.assignmentId AND hod.tenantId = ? LEFT JOIN egeis_employeeJurisdictions ej ON e.id = ej.employeeId AND ej.tenantId = ? WHERE e.tenantId = ? AND a.tenantId = ? LIMIT ? OFFSET ?";
	
		assertEquals(queryString, expectedQueryString);
		assertEquals((String)preparedStatementValues.get(0), "1"); // tenant id is 1
		assertEquals((long)preparedStatementValues.get(4), 3); // limit is 3
	...
	public void testQueryForListOfEmployeeIds() {
		...
		ids.add(100L);
		ids.add(200L);
		EmployeeCriteria employeeCriteria = new EmployeeCriteria().builder().tenantId("1").id(ids).build();
		List<Object> preparedStatementValues = new ArrayList<>();

		String queryString = employeeQueryBuilder.getQueryForListOfEmployeeIds(employeeCriteria, preparedStatementValues);
		String expectedQueryString = "SELECT distinct e.id AS id  FROM egeis_employee e JOIN egeis_assignment a ON e.id = a.employeeId LEFT JOIN egeis_hodDepartment hod ON a.id = hod.assignmentId AND hod.tenantId = ? LEFT JOIN egeis_employeeJurisdictions ej ON e.id = ej.employeeId AND ej.tenantId = ? WHERE e.tenantId = ? AND a.tenantId = ? AND e.id IN (100, 200) LIMIT ? OFFSET ?";
