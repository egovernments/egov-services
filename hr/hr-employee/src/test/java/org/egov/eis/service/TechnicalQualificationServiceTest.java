...
public class TechnicalQualificationServiceTest {
	...
	public void testUpdate(){
		...
		when(technicalQualificationRepository.findByEmployeeId(100L, "1"))
        .thenReturn(getTechnicalsInDBForEmployee());
		...
		}).when(technicalQualificationRepository).delete(Matchers.anyListOf(Long.class), Matchers.anyLong(), Matchers.anyString());
		doAnswer(new Answer<Void>() {
		    public Void answer(InvocationOnMock invocation) {
		      Object[] args = invocation.getArguments();
		...
		}).when(technicalQualificationRepository).insert(Matchers.any(TechnicalQualification.class), Matchers.anyLong());
		doAnswer(new Answer<Void>() {
		    public Void answer(InvocationOnMock invocation) {
		      Object[] args = invocation.getArguments();
