...
import org.egov.lams.common.web.request.LandAcquisitionResponse;
...
public class LandAcquisitionService {
	...
	public LandAcquisitionResponse  create(LandAcquisitionRequest landAcquisitionRequest) {
		landAcquisitionRequest.getLandAcquisition().stream().forEach(landAcquisition -> {
			landAcquisition.setId(sequenceGenService.getIds(1, "seq_eglams_landacquisition").get(0));
			
			landAcquisition.getPossessionOfLand().stream().forEach(possessionOfLand -> {
			possessionOfLand.getLandPossession().setId(sequenceGenService.getIds(1, "seq_eglams_possessionOfLand").get(0));
			});
			
			landAcquisition.getLandTransfer().stream().forEach(landTransfer -> {
			landTransfer.setId(sequenceGenService.getIds(1, "seq_eglams_landTransfer").get(0));
			});
			
			landAcquisition.getValuationDetails().setId(sequenceGenService.getIds(1, "seq_eglams_valuationDetail").get(0));
			
			landAcquisition.getProposalDetails().setId(sequenceGenService.getIds(1, "seq_eglams_propasalDetail").get(0));
			
			landAcquisition.getSupportDocuments().stream().forEach(suportDocument -> {
			suportDocument.setId(sequenceGenService.getIds(1,"seq_eglams_supportdocument").get(0));
			});
