...
public class PaymentService {
	...
	public String generateBillXml(Agreement agreement, RequestInfo requestInfo) {
		...
		try {
			...
			billInfo.setId(null);
			LOGGER.info("the demands for a agreement object"
					+ agreement.getDemands());
			if (agreement.getDemands() != null
					&& !agreement.getDemands().isEmpty()) {
				LOGGER.info("the demand id from agreement object"
						+ agreement.getDemands().get(0));
				billInfo.setDemandId(Long
						.valueOf(agreement.getDemands().get(0)));
			}
			...
			BoundaryResponse boundaryResponse = getBoundariesById(agreement
					.getAsset().getLocationDetails().getElectionWard());
			billInfo.setBoundaryNumber(boundaryResponse.getBoundarys().get(0)
					.getBoundaryNum());
			...
			demandSearchCriteria.setDemandId(Long.valueOf(agreement
					.getDemands().get(0)));
			...
			int orderNo = 0;
			System.out
					.print("PaymentService- generateBillXml - getting purpose");
	...
	public ResponseEntity<ReceiptAmountInfo> updateDemand(
		...
		System.out
				.print("PaymentService- updateDemand - billReceiptInfoReq::: - "
		...
		updateDemandDetailForReceiptCreate(currentDemand,
				billReceiptInfoReq.getBillReceiptInfo());
		System.out
				.print("PaymentService- updateDemand - updateDemandDetailForReceiptCreate done");
		LOGGER.info("The amount collected from citizen is ::: "
				+ currentDemand.getCollectionAmount());
		currentDemand.setPaymentInfos(setPaymentInfos(billReceiptInfo));
		demandRepository
				.updateDemand(Arrays.asList(currentDemand), requestInfo)
				.getDemands().get(0);
		...
		LOGGER.info("the consumer code from bill object ::: "
				+ billInfo.getConsumerCode());
	...
	private void updateWorkflow(String consumerCode, RequestInfo requestInfo) {
		...
		String sql = "select *,agreement.id as agreementid from eglams_agreement agreement "
				...
				+ " where agreement.acknowledgementnumber='"
				+ consumerCode
				+ "' OR agreement.agreement_no='" + consumerCode + "'";

		LOGGER.info("the sql query for fetching agreement using consumercode ::: "
				+ sql);
	...
	private void updateDemandDetailForReceiptCreate(Demand demand,
		...
		LOGGER.info("the size of objects ::: "
				+ billReceiptInfo.getAccountDetails().size()
				+ "the size of demand details ::"
				+ demand.getDemandDetails().size());
		for (final ReceiptAccountInfo rcptAccInfo : billReceiptInfo
				.getAccountDetails()) {

			totalAmountCollected = totalAmountCollected
					.add(updateDemandDetails(demand, rcptAccInfo));
		}
		LOGGER.info("updateDemandDetailForReceiptCreate  ::: totalAmountCollected "
				+ totalAmountCollected);
		demand.setCollectionAmount(totalAmountCollected);
	}

	private BigDecimal updateDemandDetails(Demand demand,
		...
		if (rcptAccInfo.getCrAmount() != null && rcptAccInfo.getCrAmount() > 0
			...
			for (final DemandDetails demandDetail : demand.getDemandDetails()) {
				if (demandDetail.getTaxPeriod() != null
					...
					totalAmountCollected = totalAmountCollected.add(BigDecimal
							.valueOf(rcptAccInfo.getCrAmount()));
					LOGGER.info("everytime totalAmountCollected ::: "
							+ totalAmountCollected);
				}
			}
		}
	...
	public ResponseEntity<ReceiptAmountInfo> receiptAmountBifurcation(
		...
		System.out
				.print("PaymentService- receiptAmountBifurcation - billReceiptInfo - "
						+ billReceiptInfo);
		System.out
				.print("PaymentService- receiptAmountBifurcation - billInfo - "
						+ billInfo);
		...
		System.out
				.print("PaymentService- receiptAmountBifurcation - getting purpose");
		Map<String, String> purposeMap = billRepository.getPurpose();
		final List<BillDetailInfo> billDetails = new ArrayList<>(
		...
		for (final ReceiptAccountInfo rcptAccInfo : billReceiptInfo
				.getAccountDetails()) {
			System.out
					.print("PaymentService- receiptAmountBifurcation - rcptAccInfo - "
							+ rcptAccInfo);
			if (rcptAccInfo.getCrAmount() != null
					&& BigDecimal.valueOf(rcptAccInfo.getCrAmount()).compareTo(
							BigDecimal.ZERO) == 1) {
				if (rcptAccInfo.getPurpose().equals(
						purposeMap.get("ARREAR_AMOUNT").toString()))
			...
			System.out
					.print("PaymentService- receiptAmountBifurcation - receiptAmountInfo - "
							+ receiptAmountInfo);
		}
	...
	private String getGlcodeById(Long id, RequestInfo requestInfo) {
		...
		return financialsRepository.getChartOfAccountGlcodeById(
				chartOfAccountContract, requestInfo);
	}
	...
	private BoundaryResponse getBoundariesById(Long boundaryId) {
		...
		try {
			boundaryResponse = restTemplate.getForObject(boundaryUrl,
