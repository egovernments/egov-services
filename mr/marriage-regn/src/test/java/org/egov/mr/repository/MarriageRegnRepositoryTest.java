...
public class MarriageRegnRepositoryTest {
	...
	public void testFindForCriteria() {
		...
		when(jdbcTemplate.query(any(String.class), any(Object[].class), any(MarriageRegnIdsRowMapper.class))).thenReturn( getApplicationnumbers());
		when(marriageRegnQueryBuilder.getQuery(any(MarriageRegnCriteria.class),any(List.class), any(List.class))).thenReturn(StringUtils.EMPTY);
		when(jdbcTemplate.query(any(String.class), any(Object[].class), any(MarriageRegnRowMapper.class))).thenReturn(marriageRegns);
