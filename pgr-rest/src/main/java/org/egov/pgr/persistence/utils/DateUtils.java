...
package org.egov.pgr.persistence.utils;
...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
     ...
     * @throws Exception 
     **/
    public static Date getDate(final String date, final String pattern) throws Exception {
        try {
            return isNotBlank(date) && isNotBlank(pattern) ? getDateFormatter(pattern).parse(date) : null;
        } catch (final Exception e) {
            throw new Exception("Date or Pattern value is not valid", e);
        }
    }
