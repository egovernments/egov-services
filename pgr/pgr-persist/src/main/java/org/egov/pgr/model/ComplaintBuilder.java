...
public class ComplaintBuilder {
	...
	private final ReceivingModeService receivingModeService;

	public ComplaintBuilder(Complaint complaint, SevaRequest sevaRequest, ComplaintTypeService complaintTypeService,
			ComplaintStatusService complaintStatusService, EscalationService escalationService,
			PositionRepository positionRepository, UserRepository userRepository,
			ReceivingCenterService receivingCenterService, RequestInfo requestInfo,
	...
	private void setEscalationDate() {
		...
		Long designationId = positionRepository.designationIdForAssignee(serviceRequest.getTenantId(),
				this.complaint.getAssignee());
	...
	private void setReceivingMode() {
		...
		if (receivingMode != null)
			this.complaint.setReceivingMode(receivingModeService.getReceivingModeByCode(receivingMode.toUpperCase()));
	}
	...
	private void setReceivingCenter() {
		...
		if (receivingCenter != null && !receivingCenter.isEmpty()) {
			ReceivingCenter receivingCenterDB = receivingCenterService
