...
public class ComplaintRepositoryTest {
	...
	@Test
	@Sql(scripts = {"/sql/InsertComplaintData.sql"})
	...
	public void testShouldFindAllComplaints() {
		...
		ComplaintSearchCriteria complaintSearchCriteria = ComplaintSearchCriteria.builder().assignmentId(1L)
				.serviceCode("AODTDGC").serviceRequestId("0005-2017-AB").status("REGISTERED").startDate(date.toDate()).endDate(date2.toDate())
				.lastModifiedDatetime(date4.toDate()).userId(2L).name("kumar").mobileNumber("7475844747").emailId("abc@gmail.com").receivingMode(5L).locationId(1L).childLocationId(null).build();
