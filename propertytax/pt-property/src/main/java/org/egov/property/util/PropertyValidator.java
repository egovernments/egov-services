...
public class PropertyValidator {
	...
	private PropertyResponse validateUpdateProperty(PropertyRequest propertyRequest) {
		...
		for (Property property : propertyRequest.getProperties()) {
			...
			if (acknowledgementNo == null) {
				throw new AttributeNotFoundException(env.getProperty("acknowledgement.message"),
				...
				if (workflowDetails.getAction() == null) {
					throw new InvalidUpdatePropertyException(env.getProperty("workflow.action.message"),
							propertyRequest.getRequestInfo());

				} else if (workflowDetails.getAssignee() == null) {
					throw new InvalidUpdatePropertyException(env.getProperty("workflow.assignee.message"),
							propertyRequest.getRequestInfo());

				} else if (workflowDetails.getDepartment() == null) {
					throw new InvalidUpdatePropertyException(env.getProperty("workflow.department.message"),
							propertyRequest.getRequestInfo());

				} else if (workflowDetails.getDesignation() == null) {
					throw new InvalidUpdatePropertyException(env.getProperty("workflow.designation.message"),
							propertyRequest.getRequestInfo());

				} else if (workflowDetails.getStatus() == null) {
					throw new InvalidUpdatePropertyException(env.getProperty("workflow.status.message"),
							propertyRequest.getRequestInfo());
	...
	/**
	 * Description : For each property, it checks and validate
	 * 1.PropertyBoundary 2.acknowledgementNo 3.workflowDetails Action
	 * 4.workflowDetails Assignee 5.workflowDetails Department 6.workflowDetails
	 * Designation 7.workflowDetails Status
	 */
	...
	/***
	 * Description : This method will validate property request and send to
	 * kafka server
	 * 
	 * @param propertyRequest
	 * @return propertyResponse
