...
public class TitleTransferConsumer {
    ...
    public void receive(ConsumerRecord<String, TitleTransferRequest> consumerRecord) throws Exception {
        ...
        for (User user : titleTransfer.getNewOwners()) {
            ...
            if(consumerRecord.topic().equalsIgnoreCase(environment.getProperty("egov.propertytax.property.titletransfer.create")))
            producer.kafkaTemplate.send(environment.getProperty("egov.propertytax.property.titletransfer.workflow.create"), titleTransferRequest);
      
            else if(consumerRecord.topic().equalsIgnoreCase(environment.getProperty("egov.propertytax.property.titletransfer.update")))
                producer.kafkaTemplate.send(environment.getProperty("egov.propertytax.property.titletransfer.workflow.update"), titleTransferRequest);
        }
    }
