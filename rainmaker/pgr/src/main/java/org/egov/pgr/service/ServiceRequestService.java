...
public class ServiceRequestService {
	...
	/***
	 * Asynchronous method performs business logic if any 
	 * and adds the data to persister queue on create topic
	 * @param request
	 * @return
	 */
	public ServiceReqResponse create(ServiceReqRequest request) {
		...
		setIdsForSubLists(requestInfo, mediaList, commentList, tenantId, true);
		kafkaProducer.send(saveTopic, request);
		return getServiceReqResponse(request);
	}
	
	/***
	 * Asynchronous method performs business logic if any and adds 
	 * the data to persister queue on update topic
	 * @param request
	 * @return
	 */
	public ServiceReqResponse update(ServiceReqRequest request) {
		...
		setIdsForSubLists(requestInfo, mediaList, commentList, tenantId, false);
		kafkaProducer.send(updateTopic, request);
		return getServiceReqResponse(request);
	}
	
	/** 
	 *  method to set Id generated by system using respective sequences to the media and comment list
	 * @param requestInfo
	 * @param mediaList
	 * @param commentList
	 * @param tenantId
	...
	private void setIdsForSubLists(RequestInfo requestInfo, List<Media> mediaList, List<Comment> commentList,
			String tenantId, Boolean isCreate) {
		
		int mediaLen = 0;
		int commentLen = 0;

		if (isCreate) {
			mediaLen = mediaList.size();
			commentLen = commentList.size();
		} else {
		...
		int count = 0;
		for (Media media : mediaList)
			if (media.getId() == null || isCreate)
				media.setId(mediaIdList.get(count++));

		count = 0;
		for (Comment comment : commentList)
			if (comment.getId() == null || isCreate)
				comment.setId(commentIdList.get(count++));
	}

	/**
	 * method to parse the IdGenResponse from IdgenRepo to List of String ids required by the respective methods
	 * @param requestInfo
	 * @param tenantId
	 * @param length
	 * @param idKey
