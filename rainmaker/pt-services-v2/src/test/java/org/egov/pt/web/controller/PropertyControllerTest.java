...
/*
    ...
    public void createTest(){
        ...
        when(propertyServiceMock.createProperty(request)).thenReturn(request.getProperties());

         FileUtils fileUtils = new FileUtils();
        try {
            mockMvc.perform(post("/property/_create").param("tenantId", "default")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(fileUtils.getFileContents("src/test/PropertyRequest.json"))).andExpect(status().isOk())
