package org.egov.wcms.web.models;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import org.egov.wcms.web.models.AuditDetails;
import org.egov.wcms.web.models.Boundary;
import org.springframework.validation.annotation.Validated;
import javax.validation.Valid;
import javax.validation.constraints.*;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;
import lombok.Builder;

/**
 * Captures Location of a connection
 */
@ApiModel(description = "Captures Location of a connection")
@Validated
@javax.annotation.Generated(value = "org.egov.codegen.SpringBootCodegen", date = "2018-05-03T01:09:48.367+05:30")

@Getter
@Setter
@AllArgsConstructor
@NoArgsConstructor
@Builder
public class Location   {
        @JsonProperty("id")
        private Long id = null;

        @JsonProperty("buildingName")
        private String buildingName = null;

        @JsonProperty("roadName")
        private String roadName = null;

        @JsonProperty("billingAddress")
        private String billingAddress = null;

        @JsonProperty("gisNumber")
        private String gisNumber = null;

        @JsonProperty("revenueBoundary")
        private Boundary revenueBoundary = null;

        @JsonProperty("locationBoundary")
        private Boundary locationBoundary = null;

        @JsonProperty("adminBoundary")
        private Boundary adminBoundary = null;

        @JsonProperty("auditDetails")
        private AuditDetails auditDetails = null;


}

