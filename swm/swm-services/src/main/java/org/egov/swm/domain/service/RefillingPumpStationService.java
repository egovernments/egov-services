...
public class RefillingPumpStationService {
    ...
    public RefillingPumpStationService(MdmsRepository mdmsRepository,
    ...
    public RefillingPumpStationRequest create(RefillingPumpStationRequest refillingPumpStationRequest){
        ...
        Long userId = null;
        if (refillingPumpStationRequest.getRequestInfo() != null && refillingPumpStationRequest.getRequestInfo().getUserInfo() != null
    ...
    private void validate(RefillingPumpStationRequest refillingPumpStationRequest){
        ...
        for(RefillingPumpStation refillingPumpStation : refillingPumpStationRequest.getRefillingPumpStations()) {
            ...
            if (refillingPumpStation.getTypeOfFuel() != null) {
                ...
                responseJSONArray = mdmsRepository.getByCriteria(refillingPumpStation.getTenantId(), Constants.MODULE_CODE,
                        Constants.FUELTYPE_MASTER_NAME, "code", refillingPumpStation.getTypeOfFuel().getCode(),
            ...
            if(refillingPumpStation.getTypeOfPump() != null){
                ...
                responseJSONArray = mdmsRepository.getByCriteria(refillingPumpStation.getTenantId(), Constants.MODULE_CODE,
                        Constants.OILCOMPANY_MASTER_NAME, "code", refillingPumpStation.getTypeOfPump().getCode(),
                        refillingPumpStationRequest.getRequestInfo());

                if (responseJSONArray != null && responseJSONArray.size() > 0)
                    refillingPumpStation.setTypeOfPump(mapper.convertValue(responseJSONArray.get(0), OilCompanyName.class));
                else
            ...
            if (refillingPumpStation.getLocation() != null && refillingPumpStation.getLocation().getBndryId() != null){
                ...
                Boundary boundary = boundaryRepository.fetchBoundaryById(Long.valueOf(refillingPumpStation.getLocation().getId()),
                        refillingPumpStation.getTenantId());

                if(boundary != null)
                    refillingPumpStation.setLocation(org.egov.swm.domain.model.Boundary.builder()
                            .id(String.valueOf(boundary.getId()))
                            .name(boundary.getName())
                            .boundaryNum(String.valueOf(boundary.getBoundaryNum()))
