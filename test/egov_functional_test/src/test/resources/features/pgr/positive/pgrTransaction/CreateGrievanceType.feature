Feature: Creating a Grievance Type
  ...
  Scenario Outline: Create a Grievance Type, router and register a grievance from official
    ...
    And user on Grievance screen types on grievanceTypeCode value "GT", 4 random upper case characters
    And user on Grievance screen copies the grievanceTypeName to grievanceTypeName
    And user on Grievance screen types on slaHours value 1
    ...
    And user on Grievance screen types on email value random email
  ...
  Scenario Outline: Create a Grievance Type, router and register a grievance as Citizen
...
####### Create A Grievance Category #######
    ...
    ### On Login Screen ###
    Given user on Login screen verifies signInText has visible value Sign In
    And user on Login screen types on username value 9036544535
...
#    And user on SearchGrievance screen verifies crnStatus has visible value <status>
  ...
  Scenario: Citizen login logout
    ...
    And user on Login screen clicks on signIn
    And user on Home screen will wait until the page loads
    And user on Home screen verifies profileName has visible value Akhila
