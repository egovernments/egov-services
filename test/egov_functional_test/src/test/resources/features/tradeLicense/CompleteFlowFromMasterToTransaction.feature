Feature: Completing the workflow for new license application from creating master till end of the workflow
  ...
  Scenario: Complete workflow for new license approval
    ...
    And user on TLFeeMatrixMaster screen types on UOMToRangeInFirstRow value 2000
    And user on TLFeeMatrixMaster screen types on AmountValueInFirstRow value 10
    And user on TLFeeMatrixMaster screen click on addButton
    And user on TLFeeMatrixMaster screen types on UOMToRangeInSecondRow value 4000
    And user on TLFeeMatrixMaster screen types on AmountValueInInSecondRow value 20
...
### On Homepage Screen ###
    ...
    And user on TradeLicense screen selects on ownerGender value FEMALE
...
#    And user on TradeLicense screen types on propertyAssessmentNumber value 1016000009
    ...
    And user on TradeLicense screen selects on tradeCategory value categoryName
...
#   And user on TradeLicense screen display on uom value test -->auto populated
    ...
    And user on TradeLicense screen will wait until the page loads
    And user on TradeLicense screen clicks on text value View
    And user on TradeLicense screen copies the applicationNumber to licenseApplicationNumber

    ### Forwarding application to Sanitary Inspector ###
    And user on Home screen clicks on homeButton
    And user on Home screen types on dashBoardSearch with above licenseApplicationNumber
    And user on Home screen clicks on containsText value licenseApplicationNumber
...
#    And user on TradeLicense screen types on comments value Can be approved
    ...
    And user on TradeLicense screen clicks on text value Ok
    And user on Home screen will wait until the page loads
#
#    ### Logout ###
    And Intent:LogoutIntentTest
#    And user on TradeLicense screen Logout
#    And user on TradeLicense screen refresh's the webpage
    ...
    And user on Home screen types on dashBoardSearch with above licenseApplicationNumber
    And user on Home screen clicks on containsText value licenseApplicationNumber
    And user on TradeLicense screen types on tradeValueForTheUOMInSI value 2200
    ...
    And user on TradeLicense screen selects on approver value Jayashree
#    And user on TradeLicense screen types on comments value Can be approved
    And user on TradeLicense screen clicks on text value Forward
    And user on TradeLicense screen clicks on text value Ok
    And user on Home screen will wait until the page loads
...
#    And user on TradeLicense screen refresh's the webpage
    ...
    And user on PayTaxes screen types on consumerCode value licenseApplicationNumber
    And user on PayTaxes screen clicks on text value Search

    And user on PayTaxes screen verifies text has visible value licenseApplicationNumber
    ...
    And user on TradeLicense screen types on searchWithApplicationNumber value licenseApplicationNumber
    And user on TradeLicense screen clicks on search
    And user on TradeLicense screen verifies text has visible value DJ Tools


    Scenario: Complete workflow for license rejection
...
#    And user on TradeLicense screen refresh's the webpage
    ...
    ### On Login to creator ###
      ...
      ### Cancelling the license application ###
      And user on Home screen will wait until the page loads
      And user on Home screen clicks on homeButton
      And user on Home screen clicks on homeButton
      And user on Home screen will wait until the page loads
      ...
      And user on TradeLicense screen clicks on text value Yes
      And user on Home screen will wait until the page loads
  ...
  Scenario: Reject the new license application and again finish the workflow
