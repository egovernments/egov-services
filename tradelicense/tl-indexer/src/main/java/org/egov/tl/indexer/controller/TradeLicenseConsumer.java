...
public class TradeLicenseConsumer {
	...
	public void receive(Map<String, TradeLicenseRequest> consumerRecord, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic)
			throws IOException {
		TradeLicenseRequest request = mapper.convertValue((consumerRecord.get(propertiesManager.getCreateLegacyTradeValidated())), TradeLicenseRequest.class);
		...
		for (TradeLicenseContract tlContract : request.getLicenses()) {
			...
			DocumentResult DocumentResult =	client.getClient().execute(new Index.Builder(propertyData).index(propertiesManager.getEsIndex())
					.type(propertiesManager.getEsIndexType()).id(String.valueOf(tlContract.getId()))
					.build());
			if(DocumentResult.isSucceeded())
			log.info("inserted data successfully to elasticsearch with Topic : "+propertiesManager.getCreateLegacyTradeValidated());
		}
		}
