...
public class ConnectionMasterAdapter implements ApplicationRunner {
	...
	public void run(ApplicationArguments arg0) throws Exception {
		...
		for (String eachTenant : tenantList) {
			...
			if (null != storageResponse) {
				if (null != storageResponse.getStorageReservoirs()
			...
			UsageTypeResponse subResponse = new RestTemplate().postForObject(url.toString(), request,
					UsageTypeResponse.class);
			if (null != subResponse) {
				if (null != subResponse.getUsageTypes() && subResponse.getUsageTypes().size() > 0) {
		...
		LOGGER.info(supplyTypeMap.size() + " Supply Type entries loaded :: " + sourceTypeMap.size()
				+ " Source Type entries loaded \n");
		LOGGER.info(pipeSizeMap.size() + " Pipe Size entries loaded :: " + treatmentPlantMap.size()
				+ " Water Treatment Plant entries loaded \n");
		LOGGER.info(storageReservoirMap.size() + " Water Storage Reservoir entries loaded :: " + usageTypeMap.size()
				+ " Usage Type entries loaded \n");
	...
	public static String getSourceTypeById(String sourceTypeId, String tenantId, RequestInfo requestInfo) { 
		if(sourceTypeMap.containsKey(sourceTypeId) && StringUtils.isNotBlank(sourceTypeMap.get(sourceTypeId).getTenantId()) &&
	...
	public static String getTreatmentPlantById(String treatmentPlantId, String tenantId, RequestInfo requestInfo) { 
		if(treatmentPlantMap.containsKey(treatmentPlantId) && StringUtils.isNotBlank(treatmentPlantMap.get(treatmentPlantId).getTenantId()) && 
	...
	public static String getStorageReservoiById(String storageReservoirId, String tenantId, RequestInfo requestInfo) { 
		if(storageReservoirMap.containsKey(storageReservoirId) && StringUtils.isNotBlank(storageReservoirMap.get(storageReservoirId).getTenantId()) &&
	...
	public static String getSubUsageTypeById(String subUsageTypeId, String tenantId, RequestInfo requestInfo) { 
		if(subUsageTypeMap.containsKey(subUsageTypeId) && StringUtils.isNotBlank(subUsageTypeMap.get(subUsageTypeId).getTenantId()) && 
