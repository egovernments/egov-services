...
public class TreatmentPlantRepository {
	...
	public List<TreatmentPlant> findForCriteria(final TreatmentPlantGetRequest treatmentPlantGetRequest) {
		/*
		 * final List<String> boundaryWardNumsList = new ArrayList<>(); final
		 * List<String> boundaryZoneNumsList = new ArrayList<>(); final
		 * List<String> boundaryLocationNumsList = new ArrayList<>();
		 */
		final Map<String, Object> batchArguments = new HashMap<>();
		...
		/*
		 * // fetch boundary Ward Nums and set the boundary name here for (final
		 * TreatmentPlant treatmentPlant : treatmentPlantList)
		 * boundaryWardNumsList.add(treatmentPlant.getWardNum()); final String[]
		 * boundaryWardNum = boundaryWardNumsList.toArray(new
		 * String[boundaryWardNumsList.size()]); final BoundaryResponse
		 * boundaryResponse =
		 * restExternalMasterService.getBoundaryName(WcmsConstants.WARD,
		 * boundaryWardNum, treatmentPlantGetRequest.getTenantId()); for (final
		 * TreatmentPlant treatmentPlant : treatmentPlantList) for (final
		 * Boundary boundary : boundaryResponse.getBoundarys()) if
		 * (boundary.getBoundaryNum().equals(treatmentPlant.getWardNum()))
		 * treatmentPlant.setWardName(boundary.getName());
		 * 
		 * // fetch boundary Zone Nums and set the boundary name here for (final
		 * TreatmentPlant treatmentPlant : treatmentPlantList)
		 * boundaryZoneNumsList.add(treatmentPlant.getZoneNum()); final String[]
		 * boundaryZoneNum = boundaryZoneNumsList.toArray(new
		 * String[boundaryZoneNumsList.size()]); final BoundaryResponse
		 * boundaryZone =
		 * restExternalMasterService.getBoundaryName(WcmsConstants.ZONE,
		 * boundaryZoneNum, treatmentPlantGetRequest.getTenantId()); for (final
		 * TreatmentPlant treatmentPlant : treatmentPlantList) for (final
		 * Boundary boundary : boundaryZone.getBoundarys()) if
		 * (boundary.getBoundaryNum().equals(treatmentPlant.getZoneNum()))
		 * treatmentPlant.setZoneName(boundary.getName());
		 * 
		 * // fetch boundary Location Nums and set the boundary name here for
		 * (final TreatmentPlant treatmentPlant : treatmentPlantList)
		 * boundaryLocationNumsList.add(treatmentPlant.getLocationNum()); final
		 * String[] boundaryLocationNum = boundaryLocationNumsList .toArray(new
		 * String[boundaryLocationNumsList.size()]); final BoundaryResponse
		 * boundaryLocation =
		 * restExternalMasterService.getBoundaryName(WcmsConstants.LOCALITY,
		 * boundaryLocationNum, treatmentPlantGetRequest.getTenantId()); for
		 * (final TreatmentPlant treatmentPlant : treatmentPlantList) for (final
		 * Boundary boundary : boundaryLocation.getBoundarys()) if
		 * (boundary.getBoundaryNum().equals(treatmentPlant.getLocationNum()))
