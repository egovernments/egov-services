...
class SearchAsset extends React.Component {
  ...
  handleAction(e, id, revaluationId) {
    ...
    console.log("-------------------",revaluationId);

    if(revaluationId)
      window.open(`app/asset/view-asset-depreciation.html?id=${id}&depreciationId=${revaluationId}` + (getUrlVars()["type"] ? "&type=" + getUrlVars()["type"] : ""), '_blank', 'height=760, width=800, scrollbars=yes, status=yes');
    else
      window.open(`app/asset/view-asset-depreciation.html?id=${id}` + (getUrlVars()["type"] ? "&type=" + getUrlVars()["type"] : ""), '_blank', 'height=760, width=800, scrollbars=yes, status=yes');
      ...
      commonApiPost("asset-services","assets","_search", {...tempInfo, tenantId, pageSize:500}, function(err, res) {
        if(res) {
          ...
          if(getUrlVars()["type"] == "view" && assetList.length) {
            ...
            commonApiPost("asset-services", "assets/depreciations", "_search", {assetId: assetIds, tenantId, pageSize:500}, function(err, res2) {
              if(res2) {
                if(res2.DepreciationReportCriteria && res2.DepreciationReportCriteria.length) {
                  var newArray = [];
                  for(var i=0; i<res2.DepreciationReportCriteria.length; i++) {
  ...
  render() {
    ...
    return (
      <div>
      <h3>Asset Depreciation Search</h3>
