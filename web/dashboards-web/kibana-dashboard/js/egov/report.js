...
var todayDate=Date.today().toString('yyyy-MM-ddT00:00:00.000Z'); // today date (syntax- {today})
var startDateOfMonth=Date.today().set({ day: 1, hour: 0, minute: 0 }).toString('yyyy-MM-ddT00:00:00.000Z'); // start date of month (syntax- {startOfMonth})
var tomorrowDate=Date.today().add(1).days().toString('yyyy-MM-ddT00:00:00.000Z'); // tomorrow date (syntax- {tomorrow})
...
/*var reportarray = [{"report":"Grievance Type Wise Report", "url":"/pgr/report/complaintTypeReport"},
        ...
        $('.open-popup').each(function(){
        	var report={report:$(this).html()+" - "+$(this).closest('div.panel').find('.panel-heading').html(), url:$(this).attr('href')};
        ...
        for(var prop in theObject) {
            ...
            if(prop == 'report') {
                if (theObject[prop].toLowerCase().indexOf(searchkey) >= 0){
                	...
                	searchlink+='<li><a href="'+theObject.url+'" class="open-popup" target="_blank" data-strwindname="'+theObject[prop]+'">'+theObject[prop]+'</a></li>';
                	return theObject;	
                }
            }
