...
class EmployeeSearch extends React.Component {
  ...
  componentDidMount() {
    ...
    $('#hp-citizen-title').text(titleCase(getUrlVars()["type"]) + "Search Employee for " + titleCase("nominee"));
  ...
  render() {
    ...
    const renderBody = function() {
      ...
      return employees.map((item,index)=>
      {
            ...
            return (<tr key={index}>
                    ...
                    {renderAction(getUrlVars()["type"],"nominee",item.id)}
                    </td>
                </tr>
            );
    ...
    return (
      <div>
        <h3>Search Employee for {titleCase("nominee")}</h3>
