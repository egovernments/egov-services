        ...
        <div>
          <div className="form-section">
            <form>
              ...
              <h3> <center><font color="ass"> {getUrlVars()["action"] ? (getUrlVars()["action"].toLowerCase() == "approve" ? "Agreement Approved." : (getUrlVars()["action"].toLowerCase() == "forward" ? "Agreement forwarded to " + (getUrlVars()["name"] ? decodeURIComponent(getUrlVars()["name"]) : "-") : "Agreement rejected and forwarded to " + (getUrlVars()["name"] ? decodeURIComponent(getUrlVars()["name"]) : "-"))) : "Agreement Created"} </font> </center></h3>
              <h4> <center><font color="ass"><strong> {getUrlVars()["ackNo"] ? (getUrlVars()["action"] && getUrlVars()["action"].toLowerCase() == "approve" ? "Agreement Number: " : "Acknowledgement number: ") + decodeURIComponent(getUrlVars()["ackNo"]) : ""}</strong>  </font> </center></h4>
