...
class FileInput extends Component{
    ...
    handleFileChange(e) {
      ...
      this.props.addFileHandler({isRequired:this.props.isRequired, code:this.props.code, files:[...files]}); //It's mandatory to define [...files]
    }
         ...
         files.map((file)=>{
           if(file.type.startsWith("image")){
               ...
               reader.onloadend = function (e) {
                    ...
                    previewFiles.push({
                         name:file.name, src:e.target.result
                    });
              ...
              const previewFiles=this.state.previewFiles;
              previewFiles.push({
                 name:file.name, icon:(<ActionAssignment/>)
              });
              this.setState(previewFiles);
            }
         });
    ...
    showFiles(){
      return this.props.files.map((file, index)=>{
        return (<ListItem style={styles.listItemStyle} key={index} disabled={true} leftAvatar={ this.renderAvatar(file) }
              rightIcon={
                <div>
    ...
    renderInputFile = () =>{
      if(this.props.isMultipleFile){
        return <input type="file" multiple ref={this.props.code} style={styles.inputFileStyle} onChange={this.handleFileChange}></input>;
      }
      else{
        return <input type="file" ref={this.props.code} style={styles.inputFileStyle} onChange={this.handleFileChange}></input>;
      }
    }
