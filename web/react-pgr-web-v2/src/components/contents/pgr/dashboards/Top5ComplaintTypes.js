...
export default class Top5ComplaintTypes extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      Api.commonApiPost(
        'pgr/dashboard/complainttype/topfive',
        {},
          ...
          topFiveComplaintsData = lastMonths.map(month => {
            let dataArry = legends.map(legend => {
              let complaintTypeObj =
                monthWiseData[month] &&
                monthWiseData[month].find(
                  cType => cType.ComplaintType === legend.complaintTypeName
                );
              return {
        ...
        <Grid fluid={true}>
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" aspect={4.0 / 1.0}>
              ...
              >
                <XAxis
                  tick={<CustomizedAxisTick isMonthNumber={true} />}
                  dataKey="name"
                />
                <YAxis
                ...
                <Legend
                  content={
                    <CustomizedLineChartLegend
                      colors={COLORS}
                      legends={legends}
                      legendNameKey="complaintTypeName"
                    />
                  }
                  layout="horizontal"
                  verticalAlign="top"
