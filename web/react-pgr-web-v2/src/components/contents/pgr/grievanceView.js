...
class grievanceView extends Component {
    ...
    Api.commonApiPost(
      ...
      function(response) {
        ...
        Api.commonApiPost('/workflow/history/v1/_search', {
          workflowId: currentThis.state.systemStateId,
        }).then(
          function(response) {
            //console.log(JSON.stringify(response));
            currentThis.setState({ workflow: response });

            Api.commonApiGet('/filestore/v1/files/tag', {
              tag: currentThis.state.serviceRequestId,
            }).then(
              function(response) {
                //console.log(JSON.stringify(response));
  ...
  SDAPI = () => {
    Api.commonApiPost('/pgr/servicedefinition/v1/_search', {
  ...
  getDepartmentById = () => {
    Api.commonApiPost('/egov-common-masters/departments/_search', {
      id: this.state.systemDepartmentId,
    }).then(
      function(response) {
  ...
  getReceivingCenter() {
    if (this.state.systemReceivingCenter) {
      Api.commonApiPost('/pgr-master/receivingcenter/v1/_search', {
        id: this.state.systemReceivingCenter,
      }).then(
        function(response) {
          currentThis.setState({
            receivingCenterName: response.ReceivingCenterType[0].name,
          });
  ...
  getWardbyId() {
    if (this.state.systemLocationId)
      Api.commonApiGet('/egov-location/boundarys', {
        boundary: this.state.systemLocationId,
      }).then(
        function(response) {
  ...
  getLocation() {
    if (this.state.systemChildLocationId)
      Api.commonApiGet('/egov-location/boundarys', {
        boundary: this.state.systemChildLocationId,
      }).then(
        function(response) {
          currentThis.setState({
            childLocationName: response.Boundary[0].name,
          });
  ...
  nextStatus = () => {
    if (localStorage.getItem('type')) {
      Api.commonApiPost('/workflow/v1/nextstatuses/_search', {
        currentStatusCode: this.state.systemStatus,
      }).then(
        function(response) {
  ...
  checkUpdateEnabled = () => {
    Api.commonApiPost('/pgr/seva/v1/_get', {
      crn: currentThis.props.match.params.srn,
    }).then(
      function(response) {
  ...
  getLocality = () => {
    Api.commonApiPost('/egov-location/boundarys/childLocationsByBoundaryId', {
      boundaryId: this.state.systemLocationId,
    }).then(
      function(response) {
  ...
  handleWard: (value, property, isRequired, pattern) => {
    Api.commonApiPost('/egov-location/boundarys/childLocationsByBoundaryId', {
      ...
      function(response) {
        ...
        dispatch({
          type: 'ADD_MANDATORY',
          property: 'systemChildLocationId',
          value: '',
          isRequired: true,
          pattern: '',
        });
