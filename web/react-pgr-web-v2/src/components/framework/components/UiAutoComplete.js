  ...
  componentWillReceiveProps(nextProps, nextState) {
	...
	callAPI = (keyUpValue) => {
		...
		if(item.hasOwnProperty("url") && item.url && item.url.search("\\|")>-1 && item.url.search("{")==-1)
		{
			...
			queryStringObject = queryStringObject.filter(n => {return n});
          ...
          <AutoComplete
             ...
             onKeyUp={(e) => {
							//  console.log('keyup:',e.target.value);
							 ...
							 this.props.handler({target: {value: e.target.value}}, item.jsonPath, item.isRequired ? true : false, '', item.requiredErrMsg, item.patternErrMsg);
             }}
