...
class assetCategoryCreate extends Component {
  ...
  displayUI(results) {
    ...
    if (hashLocation.split('/').indexOf('update') == 1) {
      ...
      var query = {
        [specifications[`asset.create`].searchUrl
          .split('?')[1]
          .split('=')[0]]: id,
      };
      //handle 2nd parameter
      if (
        specifications[`asset.create`].searchUrl.split('?')[1].split('=')[2]
      ) {
        var pval = specifications[`asset.create`].searchUrl
          .split('?')[1]
          .split('=')[2];
        var pname = specifications[`asset.create`].searchUrl
          .split('?')[1]
          .split('=')[1]
          .split('&')[1];
      ...
      Promise.all([
        Api.commonApiPost(
          '/egov-mdms-service/v1/_get?moduleName=ASSET&masterName=AssetCategory',
          {},
          {},
          false,
          mockObj.useTimestamp
...
*/
                        ...
                        if (res.MdmsRes[prop].hasOwnProperty(master)) {
                          ...
                          if (
                            ...
                            moduleObj[master].forEach(function(item) {
                              ...
                              masterObj.key =
                                item[obj.groups[i].fields[j].mdms.key];
                              masterObj.value =
                                item[obj.groups[i].fields[j].mdms.value];
  ...
  initData() {
    ...
    //   console.log(e);
    // }
    specifications = require(`../../../framework/specs/asset/master/createAssetCategroy`)
      .default;
  ...
  create = e => {
    ...
    if (formData.MasterMetaData) {
      ...
      formData.MasterMetaData.masterData[0].tenantId = localStorage.getItem(
        'tenantId'
      );
    }
