...
class assetImmovableCreate extends Component {
  ...
  modifyData(urlId) {
    let self = this;
    let assetCheck = {};
    let specifications = require(`../../../framework/specs/asset/master/assetImmovable`)
      .default;
      ...
      function(response) {
        for (var i = 0; i < response.Assets[0].assetAttributes.length; i++) {
          assetCheck[response.Assets[0].assetAttributes[i].key] = {
            [response.Assets[0].assetAttributes[i].type]:
              response.Assets[0].assetAttributes[i].value,
          };
        }
        if (
          response &&
          response.Assets &&
          response.Assets[0] &&
          response.Assets[0].titleDocumentsAvailable
        ) {
        ...
        self.customFieldDataFun(
          self.state.customFieldsGen[response.Assets[0].assetCategory.id]
        );
  ...
  initData() {
    ...
    specifications = require(`../../../framework/specs/asset/master/assetImmovable`)
      .default;
  ...
  componentDidMount() {
    ...
    Api.commonApiPost(
      ...
      if (
        ...
        response.MdmsRes.ASSET.Assetconfiguration[0].keyname ==
          'EnableVoucherGeneration'
      ) {
        if (
          response.MdmsRes.ASSET.Assetconfiguration[0].values[0].value == 'true'
        ) {
          self.props.addRequiredFields(['Asset.assetAccount']);
          self.props.addRequiredFields([
    ...
    Api.commonApiPost(
      '/egov-mdms-service/v1/_get',
      {
        ...
        filter:
          '%5B%3F(%20%40.isAssetAllow%20%3D%3D%20true%20%26%26%20%40.assetCategoryType%20%3D%3D%20%22IMMOVABLE%22)%5D%0A',
      },
      {},
      false,
      false,
      ...
      function(response) {
        if (response) {
          ...
          let values = jp.query(
            response,
            '$.MdmsRes.ASSET.AssetCategory.*.name'
          );
          let dropDownData = [];
          for (var k = 0; k < keys.length; k++) {
            let obj = {};
        ...
        for (var i = 0; i < response.MdmsRes.ASSET.AssetCategory.length; i++) {
          ...
          if (
            response.MdmsRes.ASSET.AssetCategory[i].assetFieldsDefination !=
            null
          ) {
            var customFieldsArray = [];
            for (
              ...
              customTemp.jsonPath =
                ...
                response.MdmsRes.ASSET.AssetCategory[i].assetFieldsDefination[j]
                  .name +
                '.' +
                response.MdmsRes.ASSET.AssetCategory[i].assetFieldsDefination[j]
                  .type;
              ...
              customTemp.isRequired =
                response.MdmsRes.ASSET.AssetCategory[i].assetFieldsDefination[
                  j
                ].isMandatory;
              customTemp.isDisabled = !response.MdmsRes.ASSET.AssetCategory[i]
                .assetFieldsDefination[j].isActive;
              ...
              if (customTemp.type == 'singleValueList') {
                if (
                  ...
                  var handleDropdown =
                    response.MdmsRes.ASSET.AssetCategory[i]
                      .assetFieldsDefination[j].values;
                  ...
                  let dropDownValue = [];
                  customTemp.url =
                    response.MdmsRes.ASSET.AssetCategory[
                      i
                    ].assetFieldsDefination[j].url;
                  ...
                  for (let i = 0; i < _splitUrl.length; i++) {
                    ...
                    query[_splitUrl[i].split('=')[0]] = _splitUrl[i].split(
                      '='
                    )[1];
                  }
                  ...
                  Api.commonApiPost(
                    ...
                    if (resp) {
                      let keys = jp.query(
                        resp,
                        '$.MdmsRes.ASSET.LayerType.*.id'
                      );
                      let values = jp.query(
                        resp,
                        '$.MdmsRes.ASSET.LayerType.*.name'
                      );
                      let others = jp.query(
                        resp,
                        '$.MdmsRes.ASSET.LayerType.*'
                      );
                      var valueHolder = [];
                      var layerData = {};
          ...
          depericiationValue[catId] =
            response.MdmsRes.ASSET.AssetCategory[i].depreciationRate;
  ...
  autoComHandler = (autoObject, path) => {
    ...
    Api.commonApiPost(
      ...
      {},
      false,
      specifications[`asset.create`].useTimestamp
    ).then(
      function(res) {
      ...
      function(response) {
        ...
        setTimeout(function() {
          if (self.props.actionName == 'update') {
            console.log('update');
            var hash =
              '/non-framework/asset/master/assetImmovableView/' +
  ...
  create = e => {
    ...
    if (formData.Asset.assetCategory.id) {
      formData.Asset.assetCategory =
        self.state.cateoryObject[formData.Asset.assetCategory.id];
    }
    ...
    if (formData && formData.Asset && formData.Asset.fundSource) {
      if (
        formData.Asset.fundSource.code == null ||
        formData.Asset.fundSource.code == ''
      ) {
  ...
  warrantyFunction = _value => {
    ...
    for (var q = 0; q < spec[`asset.${self.state.action}`].groups.length; q++) {
      if (spec[`asset.${self.state.action}`].groups[q].name == 'AssetField') {
        for (
          var l = 0;
          l < spec[`asset.${self.state.action}`].groups[q].fields.length;
          l++
        ) {
          if (
            spec[`asset.${self.state.action}`].groups[q].fields[l].name ==
            'WarrantyExpiryDate'
          ) {
            if (valueWarranty == false) {
              spec[`asset.${self.state.action}`].groups[q].fields[
                l
              ].isRequired = false;
    ...
    if (property == 'Asset.assetCategory.id') {
      if (self.state.depericiationValue[e.target.value]) {
        var newVal = Math.round(
          100 / self.state.depericiationValue[e.target.value]
        );
    ...
    if (property == 'Asset.assetAttributesCheck.Layer Type.Select') {
      for (var i = 0; i < groups[0].fields.length; i++) {
        if (
          ...
          groups[0].fields[i].jsonPath
        ) {
          groups[0].fields[i].imagePath = this.state.layerData[
            e.target.value
          ].description;
      ...
      Api.commonApiPost(
        ...
        function(response) {
          if (
            ...
            response.Assets &&
            response.Assets[0] &&
            response.Assets[0].assetAttributes
          ) {
            for (
              var i = 0;
              i < response.Assets[0].assetAttributes.length;
              i++
            ) {
              if (
                response.Assets[0].assetAttributes[i].key == 'Survey Number'
              ) {
                var _surveyNo = response.Assets[0].assetAttributes[i].value;
                self.props.handleChange(
                  { target: { value: _surveyNo } },
                  SubProperty + '.surveyNo',
                  false,
                  '',
                  '',
                  ''
                ...
                response.Assets[0].assetAttributes[i].key == 'Area of Land'
              ) {
            ...
            setTimeout(function() {
              if (
                self.props.formData.Asset.landDetails &&
                self.props.formData.Asset.landDetails.length
              ) {
                let area = 0;
                for (
                  let i = 0;
                  i < self.props.formData.Asset.landDetails.length;
                  i++
                ) {
                  area += Number(
                    self.props.formData.Asset.landDetails[i].area || 0
                  );
                }
                self.props.handleChange(
                  { target: { value: area } },
            ...
            mockData[`${moduleName}.${actionName}`] && (
              <div>
                <ShowFields
                  ...
                  useTimestamp={
                    mockData[`${moduleName}.${actionName}`].useTimestamp ||
                    false
                  }
                    ...
                    <Grid fluid>
                        ...
                        floatingLabelText={
                          <span>
                            ...
                            <span style={{ color: '#FF0000' }}>
                              {self.props.requiredFields.indexOf(
                                'Asset.assetAccount'
                              ) > -1
                                ? ' *'
                                : ''}
                      ...
                      >
                        {self.props.dropDownData &&
                          self.props.dropDownData.hasOwnProperty(
                            'Asset.assetAccount'
                          ) &&
                          self.props.dropDownData['Asset.assetAccount'].map(
                            (dd, index) => (
                              <MenuItem
                                value={dd.key}
                                key={index}
                      ...
                      <SelectField
                        ...
                        floatingLabelText={
                          <span>
                            {translate(
                              'ac.create.Depreciation.Expenses.Account'
                            )}{' '}
                            <span style={{ color: '#FF0000' }}>
                              {self.props.requiredFields.indexOf(
                                'Asset.depreciationExpenseAccount'
