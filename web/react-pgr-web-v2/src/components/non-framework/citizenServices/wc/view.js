...
class Report extends Component {
  ...
  initData() {
    specifications = require('../../../framework/specs/citizenService/wc/NewConnection')
      .default;
    ...
    if (self.props.match.params.status == 'pay') {
      ...
      if (this.props.match.params.paymentGateWayRes == 'success') {
        ...
        Api.commonApiPost(
          ...
          null,
          metaData['wc.create'].useTimestamp,
          false,
          null,
  ...
  makePayment = (res, fee) => {
    ...
    Api.commonApiPost(
      ...
      { PGRequest },
      null,
      self.props.metaData['wc.create'].useTimestamp,
      false,
      null,
      JSON.parse(localStorage.userRequest)
    ).then(
  ...
  payFee = () => {
    ...
    var ServiceRequest = { ...this.state.ServiceRequest };
    var DemandBillQuery =
      `?businessService=WC&tenantId=${localStorage.getItem(
        'tenantId'
  ...
  updateStatusAndComments = ServiceRequest => {
    ...
    if (self.state.feeAmount) {
      ...
      DemandRequest['Demands'] = Object.assign(
        [],
        self.props.metaData['wc.create'].feeDetails
      );
      DemandRequest['Demands'][0].tenantId = localStorage.getItem('tenantId');
      DemandRequest['Demands'][0].businessService = 'WC';
          ...
          if (err) {
            ...
            _docs.push({
              ...
              newFile:
                ...
                ServiceRequest.status == 'SANCTIONED'
                  ? true
                  : null,
            });
            counter--;
      ...
      <div className="Report">
                    ...
                    {self.state.role != 'CITIZEN' ? (
                      <Col xs={12} md={6}>
                        ...
                        >
                          ...
                          />
                          <MenuItem
                            value={'ESTIMATIONNOTICEGENERATED'}
                            primaryText={'Estimation Notice Generated'}
                          />
                          <MenuItem
                            value={'ESTIMATIONAMOUNTCOLLECTED'}
                            primaryText={'Estimation Amount Collected'}
                          />
                          <MenuItem
                            value={'APPROVED'}
                          ...
                          />
                          <MenuItem
                            value={'WORKORDERGENERATED'}
                            primaryText={'WorkOrder Generated'}
                          />
                          <MenuItem
                            value={'REJECTED'}
          ...
          <Row id="allCertificates">
            <Col md={10} mdOffset={1}>
              <Card id="DownloadReceipt">
                <CardHeader
                  title={<strong>Receipt for: Water Connection</strong>}
                />
                <CardText>
                  <Table
                    responsive
        ...
        >
          ...
          <div style={{ textAlign: 'center' }}>
            <UiButton
              handler={() => {
                self.updateStatusAndComments(self.state.ServiceRequest);
              }}
              item={{ label: 'Done', uiType: 'button' }}
              ui="google"
            />
          </div>
        </Dialog>
