...
class createVoucher extends Component {
  ...
  initData() {
    ...
    let endPoint = '';
    let self = this;
    specifications = require(`../../../framework/specs/egf/master/voucher`)
      .default;
  ...
  autoComHandler = (autoObject, path) => {
    ...
    Api.commonApiPost(
      ...
      {},
      false,
      specifications[`egf.create`].useTimestamp
    ).then(
      function(res) {
  ...
  addRow = (isAdd = false) => {
    ...
    if (isAdd) {
      if (
        !(
          currentData.vouchers[0].ledgers[
            currentData.vouchers[0].ledgers.length - 1
          ].debitAmount ||
          currentData.vouchers[0].ledgers[
            currentData.vouchers[0].ledgers.length - 1
          ].creditAmount
        ...
        var text =
          'Please enter Account code and (debit amount or credit amount) for the Row no.' +
          currentData.vouchers[0].ledgers.length -
          1 +
          '';
        console.log(text);
        self.props.toggleSnackbarAndSetText(
          true,
          'Please enter Account code and (debit amount or credit amount)',
          false,
          true
        );
      } else {
        ...
        self.props.handleChange(
          { target: { value: ledgers } },
          'vouchers[0].ledgers[' + currentData.vouchers[0].ledgers.length + ']'
        );
      ...
      self.props.handleChange(
        { target: { value: { ...ledgers } } },
        'vouchers[0].ledgers[0]'
      );
      self.props.handleChange(
        { target: { value: { ...ledgers } } },
        'vouchers[0].ledgers[1]'
      );
    }
  };
  ...
  removeRow = index => {
    ...
    if (currentData.vouchers[0].ledgers.length != 2) {
      ...
      self.props.toggleSnackbarAndSetText(
        true,
        "First two row's can not be deleted",
        false,
        true
      );
    }
            ...
            <Card className="uiCard">
              ...
              <CardText>
                ...
                >
                  ...
                  <tbody>
                    {formData &&
                      formData.hasOwnProperty('vouchers') &&
                      formData.vouchers[0].hasOwnProperty('ledgers') &&
                      formData.vouchers[0].ledgers.map((item, index) => {
                        ...
                        autoComplete.jsonPath =
                          'vouchers[0].ledgers[' + index + '].glcode';
                        debitAmount.jsonPath =
                          'vouchers[0].ledgers[' + index + '].debitAmount';
                        creditAmount.jsonPath =
                          'vouchers[0].ledgers[' + index + '].creditAmount';
                        return (
                          <tr key={index}>
                            <td>
