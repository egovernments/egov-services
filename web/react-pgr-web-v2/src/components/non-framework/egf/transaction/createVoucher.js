...
import UiNumberField from '../../../framework/components/UiNumberField';
import UiAutoCompleteMultiple from "../../../framework/components/UiAutoCompleteMultiple";
...
var uiTextFieldSpecDebitAmount={
  "name": "debitAmount",
  "jsonPath": "vouchers[0].ledgers[0].debitAmount",
  "pattern": "^[0-9]+(\.[0-9]{1,2})?$",
  "type": "number",
  "isRequired": false,
  "isDisabled": false,
...
var autoCompleteSpec={
       "name": "glcode",
       "jsonPath": "vouchers[0].ledgers[0].glcode",
       "pattern": "",
       "type": "autoCompelete",
       "isRequired": false,
       "isDisabled": false,
       "requiredErrMsg": "",
       "patternErrMsg": "",
       "url":"/egf-masters/chartofaccounts/_search?|$..glcode|$..glcode,$..name"
     };
      var uiTextFieldSpecCreditAmount={
        "name": "creditAmount",
        "jsonPath": "vouchers[0].ledgers[0].creditAmount",
        "pattern": "^[0-9]+(\.[0-9]{1,2})?$",
        "type": "number",
        "isRequired": false,
        "isDisabled": false,
...
class createVoucher extends Component {
  ...
  displayUI(results) {
    ...
    let obj = specifications[`egf.create`];
    reqRequired = [];
  ...
  initData() {
    ...
    let endPoint="";
    let self = this;
    specifications = require(`../../../framework/specs/egf/master/voucher`).default;
  ...
  autoComHandler = (autoObject, path) => {
    ...
    Api.commonApiPost(url, query, {}, false, specifications[`egf.create`].useTimestamp).then(function(res){
  ...
  console.log(self.props.formData);
          if(isAdd){
            if(!(currentData.vouchers[0].ledgers[currentData.vouchers[0].ledgers.length-1].debitAmount || currentData.vouchers[0].ledgers[currentData.vouchers[0].ledgers.length-1].creditAmount)){
              var text = "Please enter Account code and (debit amount or credit amount) for the Row no." + currentData.vouchers[0].ledgers.length-1 + "";
              console.log(text);
              self.props.toggleSnackbarAndSetText(true, "Please enter Account code and (debit amount or credit amount)" , false, true);
            } else {
              let ledgers = {"glcode": "", "debitAmount": "0", "creditAmount": "0", "tenantId": localStorage.tenantId, "disabled": false, "add": false};
              self.props.handleChange({target:{value:ledgers}},"vouchers[0].ledgers[" + (currentData.vouchers[0].ledgers.length) + "]");
              console.log(currentData.vouchers[0].ledgers);
            }
          } else if(!self.props.match.params.id){
            let ledgers = {"glcode": "", "debitAmount": "0", "creditAmount": "0", "tenantId": localStorage.tenantId, "disabled": false, "add": false};
            self.props.handleChange({target:{value:{...ledgers}}},"vouchers[0].ledgers[0]");
            self.props.handleChange({target:{value:{...ledgers}}},"vouchers[0].ledgers[1]");
          }
        }
        ...
        removeRow = (index) =>{
          ...
          if(currentData.vouchers[0].ledgers.length != 2){
              Ledgers = currentData.vouchers[0].ledgers;
              Ledgers.splice(index, 1);
              self.props.handleChange({target:{value:Ledgers}},"vouchers[0].ledgers");
              console.log(currentData.vouchers[0].ledgers);
        }
        else {
          self.props.toggleSnackbarAndSetText(true, "First two row's can not be deleted", false, true);
        }
        }
        ...
        {!_.isEmpty(mockData) && moduleName && actionName && mockData[`${moduleName}.${actionName}`] && <ShowFields
          ...
          <Card className="uiCard">
              <CardHeader title="Account Details">
                <FloatingActionButton mini={true} style = {style} ><ContentAdd onClick={() => {this.addRow(true)}}  /></FloatingActionButton>
              </CardHeader>
              <CardText>
              ...
              <tbody>
                ...
                {formData && formData.hasOwnProperty("vouchers") && formData.vouchers[0].hasOwnProperty("ledgers") && formData.vouchers[0].ledgers.map((item,index)=>{
                  let autoComplete = {...autoCompleteSpec};
                  let debitAmount = {...uiTextFieldSpecDebitAmount};
                  let creditAmount = {...uiTextFieldSpecCreditAmount};

                  autoComplete.jsonPath= "vouchers[0].ledgers["+index+"].glcode";
                  debitAmount.jsonPath= "vouchers[0].ledgers["+index+"].debitAmount";
                  creditAmount.jsonPath= "vouchers[0].ledgers["+index+"].creditAmount";
                  return (
                    <tr key={index}>
                      <td>
                        <UiAutoCompleteMultiple ui={"google"} getVal={this.getVal} item={autoComplete} fieldErrors={this.props.fieldErrors} handler={this.handleChange} autoComHandler={this.autoComHandler || ""}/>
                      </td>
                      <td>
                        <UiNumberField ui={"google"} getVal={this.getVal} item={debitAmount} fieldErrors={this.props.fieldErrors} handler={this.handleChange}/>
                      </td>
                      <td>
                      <UiNumberField ui={"google"} getVal={this.getVal}
                      item={creditAmount}
                      fieldErrors={this.props.fieldErrors} handler={this.handleChange}/>
                      </td>
                      <td>
                      <FloatingActionButton disabled = {index == 0 || index == 1} mini={true}>
                      <ContentRemove disabled = {index == 0 || index == 1}
