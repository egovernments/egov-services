...
class actualKpiCreate extends Component {
  state = {
    pathname: '',
  };
  constructor(props) {
    ...
    if (property == 'kpiValues[0].KPI.code') {
      let postData = {
        RequestInfo: {
          ...
          msgId: '654654',
          requesterId: '61',
          authToken: '00ea3217-634a-41ed-8b7a-69f91cf35a29',
        },
      };
      ...
      Api.commonApiPost('/perfmanagement/v1/kpimaster/_search', {}, {}, false, true)
        .then(res => {
          ...
          for (var i = 0; i < data.length; i++) {
            if (data[i].code == e.target.value) {
              //console.log(data[i].documentsReq[0]);
              if (data[i].documentsReq.length) {
                let attachment = {
                  target: {
              ...
              let object2 = {
                target: {
                  value: data[i].targetType,
                },
              };
              handleChange(object2, 'kpiValues[0].targetType', '', '', '', '');
              handleChange(e, 'kpiValues[0].KPI.code', '', '', '', '');
              if (data[i].targetType) {
                let object3 = {
                  target: {
                    value: data[i].targetValue,
                  },
                };
...
export default connect(mapStateToProps, mapDispatchToProps)(actualKpiCreate);
