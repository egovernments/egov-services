...
class createSubCategory extends Component {
  ...
  initData() {
    ...
    specifications = require(`../../../../framework/specs/tl/master/CreateLicenseSubCategory`)
      .default;
  ...
  handleChange = (
...

    {
      formData &&
        ...
        formData.categories[0].hasOwnProperty('details') &&
        formData.categories[0].details.map((item, index) => {
          if (property == 'categories[0].details[' + index + '].feeType') {
            handleChange(
              { target: { value: null } },
              'categories[0].details[' + index + '].rateType'
            );
            handleChange(
              { target: { value: null } },
              'categories[0].details[' + index + '].uom'
            );
          }
        });
    }
