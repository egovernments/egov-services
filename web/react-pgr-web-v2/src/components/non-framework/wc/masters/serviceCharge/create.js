...
class createFeeMatrix extends Component {
  ...
  displayUI(results) {
    ...
    if (self.props.match.params.id) {
      ...
      Api.commonApiPost(
        ...
        {},
        false,
        specifications[`wc.update`].useTimestamp
      ).then(
        function(res) {
          if (specifications[`wc.update`].isResponseArray) {
            ...
            _.set(
              obj,
              specifications[`wc.update`].objectName,
              jp.query(res, '$..[0]')[0]
            );
  ...
  initData() {
    ...
    //   console.log(e);
    // }
    specifications = require(`../../../../framework/specs/wc/master/serviceCharge`)
      .default;
  ...
  calculatechargeDetails = (isAdd = false) => {
    ...
    if (isAdd) {
      if (
        currentData.ServiceCharges[0].chargeDetails[
          currentData.ServiceCharges[0].chargeDetails.length - 1
        ].uomTo &&
        currentData.ServiceCharges[0].chargeDetails[
          currentData.ServiceCharges[0].chargeDetails.length - 1
        ].amountOrpercentage
        ...
        if (
          ...
          self.props.handleChange(
            { target: { value: chargeDetails } },
            'ServiceCharges[0].chargeDetails[' +
              currentData.ServiceCharges[0].chargeDetails.length +
              ']'
          );
          ...
          self.props.toggleSnackbarAndSetText(
            true,
            'UOM To value should be greater than UOM From value',
            false,
            true
          );
        }
        ...
        self.props.toggleSnackbarAndSetText(
          true,
          'Please enter UOM To and Amount',
          false,
          true
        );
      }
      ...
      self.props.handleChange(
        { target: { value: chargeDetails } },
        'ServiceCharges[0].chargeDetails[0]'
      );
    }
            ...
            <Card className="uiCard">
              <CardHeader
                ...
                title={
                  ...
                  >
                    {translate('wc.serviceCharge.table.title')}
                  </div>
                }
              />
              <CardText>
                <div style={{ textAlign: 'right', padding: '15px' }}>
                  <FloatingActionButton mini={true}>
                    <ContentAdd
                      onClick={() => {
                        this.calculatechargeDetails(true);
                      }}
                    />
                ...
                >
                  <thead>
                    <tr>
                      <th>
                        {translate('tl.create.groups.feeMatrixDetails.uomFrom')}
                      </th>
                      <th>
                        {translate('tl.create.groups.feeMatrixDetails.uomTo')}
                      </th>
                      <th>
                        {translate('tl.create.groups.feeMatrixDetails.amount')}
                      </th>
                      <th />
                    </tr>
                  </thead>
                  <tbody>
                    {formData &&
                      formData.hasOwnProperty('ServiceCharges') &&
                      formData.ServiceCharges[0].hasOwnProperty(
                        'chargeDetails'
                      ) &&
                      formData.ServiceCharges[0].chargeDetails.map(
                        (item, index) => {
                          return (
                            <tr key={index}>
                              <td>{item.uomFrom}</td>
                                ...
                                <TextField
                                  ...
                                  onChange={e =>
                                    handleChange(
                                      ...
                                      '^[0-9]{1,10}?$',
                                      '',
                                      'Enter value greater than UOM From (Number only)'
                                    )
                                  }
                                />
                              </td>
                              ...
                              <td>
                                <FloatingActionButton
                                  disabled={
                                    index == 0 ||
                                    formData.ServiceCharges[0].chargeDetails[
                                      index + 1
                                    ]
                                  }
                                  mini={true}
