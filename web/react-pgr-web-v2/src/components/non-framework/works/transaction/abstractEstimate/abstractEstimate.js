...
class AbstractEstimate extends Component {
  ...
  initData() {
    ...
    if (hashLocation.split('/').indexOf('update') >= 1) {
      ...
      var id =
        self.props.match.params.code &&
        decodeURIComponent(self.props.match.params.code);
      var query = {
        [specifications[`works.update`].searchUrl
          .split('?')[1]
          .split('=')[0]]: id,
      };
      Promise.all([
        Api.commonApiPost(
          ...
          {},
          false,
          specifications[`works.update`].useTimestamp
        ),
      ]).then(responses => {
        try {
      ...
      handleChange(
        new Date().valueOf(),
        `${obj.objectName}[0].dateOfProposal`,
        true
      );
      ...
      sanctionType.map((sanction, index) => {
        for (var key in sanction) {
          handleChange(
            sanction['key'],
            `${obj.objectName}[0].sanctionDetails[${index}].sanctionType`,
            false
          );
          handleChange(
            '',
            `${
              obj.objectName
            }[0].sanctionDetails[${index}].sanctionAuthority.name`,
            false
          );
        }
      });
    ...
    this.props.addRequiredFields([
      `${obj.objectName}[0].workFlowDetails.department`,
      `${obj.objectName}[0].workFlowDetails.designation`,
      `${obj.objectName}[0].workFlowDetails.assignee`,
    ]);
      ...
      for (
        ...
        if (
          ...
          _mockData[moduleName + '.' + actionName].groups[i].fields[j]
            .checkMandatory &&
          _mockData[moduleName + '.' + actionName].groups[i].fields[j]
            .checkMandatory.length
        ) {
          let obj = _mockData[moduleName + '.' + actionName].groups[i].fields[
            j
          ].checkMandatory.find(checkmandat => {
                ...
                for (let a = 0; a < temp.length; a++) {
                  for (let b = 0; b < temp[a].fields.length; b++) {
                    for (let mockkey in temp[a].fields[b]) {
                      if (
                        mockkey == 'jsonPath' &&
                        temp[a].fields[b][mockkey] == obj.required[k][key]
                      ) {
                ...
                for (let a = 0; a < temp.length; a++) {
                  for (let b = 0; b < temp[a].fields.length; b++) {
                    for (let mockkey in temp[a].fields[b]) {
                      if (
                        mockkey == 'jsonPath' &&
                        temp[a].fields[b][mockkey] == obj.notRequired[k][key]
                      ) {
  ...
  renderActions = () => {
    ...
    return (
      ...
      this.state.processInstance.attributes.validActions.values.map(
        (item, index) => {
          if (item.key == 'Reject') {
            let comments =
              formData &&
