...
class ViewFunction extends Component {
  ...
  componentDidMount() {
    ...
    Api.commonApiPost("egf-masters/functions/_search",{id:1}).then(function(response)
    {
                                ...
                                <Row>
                                  <Col xs={12} md={6}>
                                    <TextField errorText={fieldErrors.name
                                      ? fieldErrors.name
                                      : ""} value={viewFunction.name?viewFunction.name:""} onChange={(e) => handleChange(e, "name", false, "")} hintText="Name" floatingLabelText="Name" />
                                  </Col>
                                  <Col xs={12} md={6}>
                                  <TextField errorText={fieldErrors.code
                                    ? fieldErrors.code
                                    : ""} value={viewFunction.code?viewFunction.code:""} onChange={(e) => handleChange(e, "code", false, "")} hintText="Code " floatingLabelText="Code" />
                                  </Col>
                                  <Col xs={12} md={6}>
                                  <br/>
                                  <br/>
                                  ...
                                  <Col xs={12} md={6}>
                                    <SelectField errorText={fieldErrors.parentType
                                      ? fieldErrors.parentType
                                      : ""} value={viewFunction.parentType?viewFunction.parentType:""} onChange={(event, index, value) => {
                                        var e = {
                                          target: {
                                            value: value
                                          }
...
const mapStateToProps = state => ({viewFunction: state.form.form, fieldErrors: state.form.fieldErrors, isFormValid: state.form.isFormValid});
