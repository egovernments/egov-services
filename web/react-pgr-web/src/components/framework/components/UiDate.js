            ...
            floatingLabelFixed={true}
						...
						onChange={(e) =>{
								if (new Date(e.target.value.split("-")[0])>1903) {
									this.props.handler({target:{value:new Date(e.target.value.split("-")[0],e.target.value.split("-")[1],e.target.value.split("-")[2]).getTime()}}, item.jsonPath, item.isRequired ? true : false, item.pattern, item.requiredErrMsg, item.patternErrMsg)
								  }
								}
						 } />
				);
