...
export default class ShowFields extends Component {
          ...
          <Grid fluid={group.isFullWidth || false} style={{paddingTop:0}}>
            <Row>
              {group.fields.map((field, fieldIndex)=>{
                  if(!field.isHidden) {
                    return (
                      <Col key={fieldIndex} xs={12} sm={field.type === "documentList" || field.type === "viewDocuments" || field.type === "fileTable" || field.type === "tableList" || field.type === "nestedTableList" || (field.type === "textarea" && field.fullWidth === true) || field.type === "workflow" ? 12 : noCols} md={field.type === "documentList" || field.type === "viewDocuments" || field.type === "fileTable" || field.type === "tableList" || field.type === "nestedTableList" || (field.type === "textarea" && field.fullWidth === true) || field.type==="window" || field.type === "workflow" ? 12 : noCols}>
  ...
  renderField=(item, screen, index,screenView)=> {
    if(screen == "view" && ["viewDocuments","documentList", "fileTable", "arrayText", "arrayNumber", "tableList", "workflow"].indexOf(item.type) == -1 ) {
  	...
  	switch(item.type) {
  		case 'text':
  			 return <UiTextField tabIndex={index} ui={this.props.ui} getVal={this.props.getVal} makeAjaxCall={this.props.makeAjaxCall} item={item}  fieldErrors={this.props.fieldErrors} handler={this.props.handler}/>
