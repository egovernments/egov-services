        ...
        for(let k=0; k<_mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields.length; k++) {
          if(val == _mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].ifValue) {
            for(let y=0; y<_mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].hide.length; y++) {
              _mockData = this.hideField(moduleName,actionName,_mockData, _mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].hide[y]);
            }

            for(let z=0; z<_mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].show.length; z++) {
              _mockData = this.showField(moduleName,actionName,_mockData, _mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].show[z]);
            }
          } else {
            for(let y=0; y<_mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].hide.length; y++) {
              _mockData = this.hideField(moduleName,actionName,_mockData, _mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].hide[y], true);
            }

            for(let z=0; z<_mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].show.length; z++) {
              _mockData = this.showField(moduleName,actionName,_mockData, _mockData[moduleName + "." + actionName].groups[i].fields[j].showHideFields[k].show[z], true);
            }
          }
        }
      }
...
const RenderGroups=props=>{
  ...
  let {groups,noCols,uiFramework,useTimestamp,jsonPath,screen,handleChange,fieldErrors,formData,getVal,changeExpanded,groupStatus,addNewCard,removeCard,autoComHandler}=props;
  switch (uiFramework) {
    case "google":
      return groups.map((group, groupIndex)=>{
        return (<Card style={{"display": group.hide ? "none" : "block"}} className="uiCard" key={groupIndex} expanded={groupStatus[group.name] ? false : true} onExpandChange={() => {changeExpanded(group.name)}}>
                          ...
                          if(!field.isHidden) {
                            return (
                              <Col key={fieldIndex} xs={12} md={noCols}>
                                  ...
                                  <RenderField field={field} screen={""} handler={handleChange} fieldErrors={fieldErrors} ui={uiFramework} formData={formData} getVal={getVal} autoComHandler={autoComHandler}/>
                              </Col>
                            )
                          }
                      })}
                  ...
                  <div style={{"marginLeft": "15px"}}>
                    {
                      ...
                      group.children.map(function(child) {
                        return (
                          <RenderGroups groups={groups} getVal={getVal} addNewCard={addNewCard} removeCard={removeCard} changeExpanded={changeExpanded} noCols={noCols} uiFramework={uiFramework?uiFramework:"google"} useTimestamp={useTimestamp} jsonPath={jsonPath?jsonPath:""} fieldErrors={fieldErrors} handleChange={handleChange}/>
                        )
                      }) : ""
                    }
                  </div>
...
class ShowFields extends Component {
  ...
  autoComHandler = (autoObject, path) => {
    ...
    Api.commonApiPost(url, query, {}, false, mockData[`${hashLocation.split("/")[2]}.${hashLocation.split("/")[1]}`].useTimestamp).then(function(res){
  ...
  addNewCard = (group, jsonPath) => {
    ...
    //Push to the path
    var updatedSpecs = getNewSpecs(moduleName,actionName,group, JSON.parse(JSON.stringify(mockData)),getPath(mockData,moduleName,actionName,jsonPath));
