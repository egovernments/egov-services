...
export default class Acknowledgement extends Component{
  ...
  doInitialStuffs = ()=>{
    ...
    Promise.all([
      ...
      this.generatePdf(response[0].image, response[1].image, response[2].TLConfiguration, "Roha Municipal");
    }).catch(function(err) {
      ...
      content: [
        ...
        },
        {
          table: {
            ...
            body: [
              [
                {text : `${translate('tl.search.groups.applicationNumber')}`}, {text:':', alignment:'left'}, {text:`${license.applicationNumber}`, alignment:'left'},
                {text : `${translate('tl.acknowledgement.applicantName')}`}, {text:':', alignment:'left'}, {text:`${license.ownerName}`, alignment:'left'}
              ],
              [
                {text : `${translate('tl.acknowledgement.serviceName')}`}, {text:':', alignment:'left'}, {text:`New License`, alignment:'left'},
                {text : `${translate('tl.acknowledgement.departmentName')}`}, {text:':', alignment:'left'}, {text:`${certificateConfigDetails['default.citizen.workflow.initiator.department.name']}`, alignment:'left'}
              ],
              [
                {text : `${translate('tl.acknowledgement.applicationFee')}`}, {text:':', alignment:'left'}, {text:``, alignment:'left'},
                '','',''
              ],
              [
                {text : `${translate('tl.acknowledgement.applicationDate')}`}, {text:':', alignment:'left'}, {text:`${epochToDate(license.applicationDate)}`, alignment:'left'},
                {text : `${translate('tl.acknowledgement.applicationTime')}`}, {text:':', alignment:'left'}, {text: `${epochToTime(license.applicationDate)}`, alignment : 'left'}
              ],
              [
                {text : `${translate('tl.acknowledgement.dueDate')}`}, {text:':', alignment:'left'}, {text:'', alignment:'left'},
                {text : `${translate('tl.acknowledgement.dueTime')}`}, {text:':', alignment:'left'}, {text: '', alignment : 'left'}
              ]
            ]
          },
          layout: 'noBorders',
