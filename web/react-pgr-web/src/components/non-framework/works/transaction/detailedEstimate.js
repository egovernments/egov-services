...
class DetailedEstimate extends Component {
  ...
  initData() {
    ...
    Api.commonApiPost('/works-masters/v1/scheduleofrates/_search').then((response)=>{
      if(response) {
    ...
    Api.commonApiPost('/works-estimate/v1/abstractestimates/_search', {abstractEstimateNumbers:"AE/ACC/000071/11/2017-18"}).then(function(response) {
      console.log('----locality----'+response.abstractEstimates[0].ward.code);
      handleChange (response.abstractEstimates[0].abstractEstimateDetails[0].projectCode.code, 'detailedEstimates[0].projectCode.code', "","","","");
      handleChange (new Date(), 'detailedEstimates[0].estimateDate', "","","","");
      handleChange ({target:{value:response.abstractEstimates[0].abstractEstimateNumber}}, 'detailedEstimates[0].estimateNumber', "","","","");
      ...
      <div className="Report">
        ...
        {!_.isEmpty(mockData) && moduleName && actionName && mockData[`${moduleName}.${actionName}`] && <ShowFields
          ...
          <Card style={styles.marginStyle}>
            <CardHeader style={{paddingTop:4,paddingBottom:0}} title={<div style={{color:"#354f57", fontSize:18,margin:'8px 0'}}>{translate('works.create.group.title.sor')}</div>}/>
            <CardText>
            <SelectField
              className="custom-form-control-for-select" fullWidth={false}
