...
createActivate = () => {
	...
	if(dataEntry.hasOwnProperty('propertyType') && dataEntry.propertyType == "PTYPE_OPEN_LAND") {
		...
		if(isFormValid && (dataEntry.floors ? (dataEntry.floors.length == 0 ? false : true) : false ) && (dataEntry.owners ? (dataEntry.owners.length == 0 ? false : true) : false )){
			notValidated = false;
		} else {
			notValidated = true;
		}
  ...
  initForm: () => {
    dispatch({
      ...
      validationData: {
        required: {
          current: [],
          required: ['reasonForCreation','propertyType','pin' ,'usage','doorNo', 'zoneNo', 'wardNo', 'sequenceNo', 'oldUpicNumber', 'totalFloors', 'currentAssessmentDate', 'firstAssessmentDate', 'lastAssessmentDate']
        },
        pattern: {
          current: [],
          required: []
