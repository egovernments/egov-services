...
class WaterSourceTypeCreate extends Component {
    ...
    submitForm = (e) => {
      ...
      if(this.props.match.params.id){
          ...
          Api.commonApiPost("/wcms/masters/sourcetype/"+body.sourceType.code+"/_update",{},body).then(function(response){
          ...
          Api.commonApiPost("/wcms/masters/sourcetype/_create",{},body).then(function(response){
              console.log(response);
              current.setState({
                open: true
              });
        ...
        <div className="createWaterSourceType">
          <form autoComplete="off" onSubmit={(e) => {submitForm(e)}}>
              <Card style={styles.marginStyle}>
                  <CardHeader  style={{paddingBottom:0}} title={< div style = {styles.headerStyle} > {this.state.id != '' ? 'Update Water Source Type ' : 'Create Water Source Type'} < /div>} />
                          ...
                          <Row>
                              <Col xs={12} md={3} sm={6}>
                                  <TextField
                                      ...
                                      maxLength={100}
                                      onChange={(e) => { createWaterSourceType.active = true;
          ...
          <Dialog
               title={this.state.id != '' ? "Water Source Type "+createWaterSourceType.name+" Updated Successfully" : "Water Source Type "+createWaterSourceType.name+" Created Successfully"}
               actions={<FlatButton
...
const mapStateToProps = state => {
  return ({createWaterSourceType : state.form.form, files: state.form.files, fieldErrors: state.form.fieldErrors, isFormValid: state.form.isFormValid,isTableShow:state.form.showTable,buttonText:state.form.buttonText});
}
