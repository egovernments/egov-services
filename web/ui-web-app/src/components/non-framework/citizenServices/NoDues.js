  ...
  handleOpen = () => {
    ...
    /*if (applicationFeeDemand[0].id==null || applicationFeeDemand[0].id=="") {
       ...
       demandReq["Demands"][0].demandDetails[0].taxHeadMasterCode=(this.props.match.params.status == "extract" ? "PT_EXT_OF_PROP_COPY_CHAR" : (this.props.match.params.id == "pt" ? "PT_NO_DUE_CERT_CHAR" : "WC_NO_DUE_CERT_CHAR"));
        ...
        if (demands.length>0) {
          Api.commonApiPost("/billing-service/bill/_generate", {businessService: self.props.match.params.id.toUpperCase(),consumerCode:formData.consumerCode}, {}, null, self.props.metaData["noDues.search"].useTimestamp,false,localStorage.getItem("auth-token-temp")).then(function(res){
              // self.props.setLoadingStatus('hide');
      ...
      Api.commonApiPost("/citizen-services/v1/pgresponse/_validate", {}, {PGResponse:paymentGateWayRes}, null, metaData["noDues.search"].useTimestamp, false, null, JSON.parse(localStorage.userRequest)).then(function(res){
          ...
          Api.commonApiPost("/collection-services/receipts/_create", {}, {"Receipt":ReceiptOne}, null, metaData["noDues.search"].useTimestamp,false,localStorage.getItem("auth-token-temp")).then(function(res1){
            if (demands.length>0) {
                  ...
                  Api.commonApiPost("/collection-services/receipts/_search", {consumerCode:formData.consumerCode,businessService:formData.businessService}, {}, null, metaData["noDues.search"].useTimestamp,false,localStorage.getItem("auth-token-temp")).then(function(res2){
                  self.props.setLoadingStatus('hide');
                  self.getProperty(function(property) {
                    self.setState({
                      Receipt: res2.Receipt,
