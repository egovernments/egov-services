          ...
          return(
            <Col xs={12} sm={4} md={3} lg={3}>
              ...
              {obj.attribValues && obj.attribValues.map((dd, index) => (
                  ...
                  checked={this.props.value && this.props.value.indexOf(dd.key) > -1} key={index} primaryText={translate(dd.name)} /> : null
              ))}
              </SelectField>
            </Col>
          );
...
class CustomAutoComplete extends Component{
   ...
   onKeyUpHandler = (e) =>{
     ...
     if(e.target.value && e.target.value !== this.state.prevText){
       ...
       Api.commonApiGet(url, queryParams).then(function(response)
       {
