...
const GisMapView = _.flowRight(
  ...
  if(props.boundaryInfoWindowOpened){
    let ward = _.find(props.wardsPolygons, function(w){ return w.boundaryCode == props.boundaryInfoWindowOpened.boundaryCode });
    if(ward){
  ...
  const renderPolygons = props.wardsPolygons && props.wardsPolygons.map((ward)=>{
      return ward.polygons.map((polygon, idx)=>{

        onPolygonClick.bind(this, ward.boundaryCode)

         return (
           <Polygon key={idx} onClick={(e)=> onPolygonClick(e, ward)}
              options={ward.style} paths={polygon}>
          </Polygon>);
      });
  });
