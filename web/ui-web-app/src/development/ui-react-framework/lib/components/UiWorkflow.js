	...
	constructor(props) {
   	...
   	initData = (props) => {
   		...
   		if(workflowId) {
		    ...
		    Api.commonApiPost("egov-common-workflows/history", { 
		    ...
		    //Fetch buttons
		    Api.commonApiPost("egov-common-workflows/process/_search", {
		    	id: workflowId
   				...
   				{!this.state.hide && <Row>
         			<Col xs={12} md={3}>
         				<SelectField 
			                ...
			                onChange={(event, key, value) => {
				                ...
				                {
				                    this.state.departments && this.state.departments.map(function(v, i){
         				...
         				<SelectField
         					className="custom-form-control-for-select"
		                  	...
		                  	onChange={(event, key, value) => {
		                    	...
		                    	{
		                      		this.state.designations && this.state.designations.map(function(v, i){
		                        		return (<MenuItem value={v.id} key={i} primaryText={v.name}/>)
		                      		})
		                  		}
		                </SelectField>
         			...
         			<Col xs={12} md={3}>
         				<SelectField
			                ...
			                onChange={(event, key, value) => {
			                  	this.props.handler({target: {value: this.getPosition(value)}}, item.jsonPath.assigneePath, true, item.requiredErrMsg, item.patternErrMsg)
			                }}>
			                    {
			                      this.state.employees && this.state.employees.map(function(v, i){
			                        return (<MenuItem value={v.id} key={i} primaryText={v.name}/>)
			                      })
			                  	}
		                </SelectField>
		         	...
		         	{ this.state.buttons.map(function(v, i){
		         		return (
		         			<span>
		         				<RaisedButton 
		         					onClick={
		         						(e) => {this.props.initiateWF(v, item, this.state.hide, this.state.status)}
		         					} 
		         					label={v.name} 
		         					primary={true}/>
		         				&nbsp;&nbsp;
