...
class ShowDocumentName extends Component {
  constructor(props) {
       super(props);
       this.state = {
  ...
  render() {
    ...
    return (
      <div className="showDocumentName">
        <form onSubmit={(e) => {
          search(e)
        }}>
          <Card>
            <CardHeader title={< strong style = {{color:"#5a3e1b"}} > Modify Document Name  < /strong>}/>
                  ...
                  <Grid>
                    ...
                    <Col xs={12} md={6}>
                      <TextField errorText={fieldErrors.name
                        ? fieldErrors.name
                        : ""} value={showDocumentName.name?showDocumentName.name:""} onChange={(e) => handleChange(e, "name", false, "")} hintText="Name" floatingLabelText="Name" />
                    </Col>
...
const mapStateToProps = state => ({showDocumentName: state.form.form, fieldErrors: state.form.fieldErrors, isFormValid: state.form.isFormValid,isTableShow:state.form.showTable,buttonText:state.form.buttonText});
...
export default connect(mapStateToProps, mapDispatchToProps)(ShowDocumentName);
