...
class ShowPropertyPipeSize extends Component {
  constructor(props) {
       super(props);
       this.state = {
  ...
  render() {
    ...
    let {search} = this;
    console.log(showPropertyPipeSize);
    ...
    return (
      <div className="showPropertyPipeSize">
        <form onSubmit={(e) => {
          search(e)
        }}>
          <Card>
            <CardHeader title={< strong style = {{color:"#5a3e1b"}} > Modify Property Pipe Size  < /strong>}/>
                  ...
                  <Grid>
                    ...
                    <Col xs={12} md={6}>
                      <TextField errorText={fieldErrors.name
                        ? fieldErrors.name
                        : ""} value={showPropertyPipeSize.name?showPropertyPipeSize.name:""} onChange={(e) => handleChange(e, "name", false, "")} hintText="Name" floatingLabelText="Name" />
                    </Col>
...
const mapStateToProps = state => ({showPropertyPipeSize: state.form.form, fieldErrors: state.form.fieldErrors, isFormValid: state.form.isFormValid,isTableShow:state.form.showTable,buttonText:state.form.buttonText});
...
export default connect(mapStateToProps, mapDispatchToProps)(ShowPropertyPipeSize);
