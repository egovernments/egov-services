...
import org.egov.works.estimate.web.contract.ProjectCodeHelper;
...
public class ProjectCodeJdbcRepository extends JdbcRepository {
    ...
    public List<ProjectCode> search(ProjectCodeSearchContract projectCodeSearchContract) {
        ...
        if (projectCodeSearchContract.getSortBy() != null && !projectCodeSearchContract.getSortBy().isEmpty()) {
            ...
            validateEntityFieldName(projectCodeSearchContract.getSortBy(), ProjectCodeHelper.class);
        }
        ...
        if (projectCodeSearchContract.getDetailedEstimateNumbers() != null
                && projectCodeSearchContract.getDetailedEstimateNumbers().size() == 1) {
            addAnd(params);
            params.append("code in (:detailedEstimateNumbers)");
            paramValues.put("detailedEstimateNumbers",
                    projectCodeSearchContract.getDetailedEstimateNumbers().get(0).toUpperCase());
        ...
        if (projectCodeSearchContract.getAbstractEstimateNumbers() != null
                && projectCodeSearchContract.getAbstractEstimateNumbers().size() == 1) {
            addAnd(params);
            params.append("code in (:abstractEstimateNumbers)");
            paramValues.put("abstractEstimateNumbers",
                    projectCodeSearchContract.getAbstractEstimateNumbers().get(0).toUpperCase());
        ...
        BeanPropertyRowMapper row = new BeanPropertyRowMapper(ProjectCodeHelper.class);

        List<ProjectCodeHelper> projectCodeHelpers = namedParameterJdbcTemplate.query(searchQuery.toString(), paramValues, row);

        List<ProjectCode> projectCodes = new ArrayList<>();
        for (ProjectCodeHelper projectCodeHelper : projectCodeHelpers) {
            projectCodes.add(projectCodeHelper.toDomain());
        }
        return projectCodes;
    }
