...
public class Error {
	 ...
	 * All services must declare their possible Error Codes with brief
	 * description in the error response section of their API path.
	 * 
	 * @return code
	 **/
	 ...
	 * call to get the other locale description if configured with the service.
	 * Clients may choose to cache these locale specific messages to enhance
	 * performance with a reasonable TTL (May be defined by the localization
	...
	/**
	 * Some error messages may carry replaceable fields (say $1, $2) to provide
	 * more context to the message. E.g. Format related errors may want to
	 * indicate the actual field for which the format is invalid. Client's
