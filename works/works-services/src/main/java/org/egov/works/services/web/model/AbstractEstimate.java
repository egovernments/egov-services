...
public class AbstractEstimate   {
 ...
 @Pattern(regexp="[a-zA-Z0-9-\\\\]") @Size(min=3,max=20)
 ...
 @Pattern(regexp="[0-9a-zA-Z_@./#&+-/!(){}\",^$%*|=;:<>?`~ ]") @Size(min=3,max=1024)
 ...
 @Pattern(regexp="[a-zA-Z0-9-\\\\]") @Size(min=3,max=50)
  ...
  **/
  ...
  public AbstractEstimate billsCreated(Boolean billsCreated) {
   ...
   /**
   * Boolean value if bills created or not. Required only if Abstract Estimate is Spill Over
   * @return billsCreated
  **/
  ...
  **/
  ...
  @Override
  public int hashCode() {
    return Objects.hash(id, tenantId, abstractEstimateNumber, subject, fund, referenceType, referenceNumber, function, description, budgetHead, scheme, subScheme, dateOfProposal, department, adminSanctionNumber, adminSanctionDate, adminSanctionBy, abstractEstimateDetails, documentDetails, status, beneficiary, modeOfAllotment, typeOfWork, subTypeOfWork, natureOfWork, ward, technicalSanctionNumber, technicalSanctionDate, technicalSanctionBy, locality, workCategory, councilResolutionNumber, councilResolutionDate, workOrderCreated, billsCreated, spillOverFlag, cancellationReason, cancellationRemarks, detailedEstimateCreated, workFlowDetails, stateId, asset, implementationPeriod, fundAvailable, fundSanctioningAuthority, pmcRequired, pmcType, pmcName, auditDetails);
  }
