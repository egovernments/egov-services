...
public interface LetterofacceptancesApi {
    ...
    @RequestMapping(value = "/letterofacceptances/_create", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/letterofacceptances/_search", method = RequestMethod.POST)
    ResponseEntity<LetterOfAcceptanceResponse> letterofacceptancesSearchPost(
    ...
    @ApiOperation(value = "Update existing Letter Of Acceptance(s).", notes = "To update existing Letter Of Acceptance in the system. API supports bulk updation with max limit as defined in the Letter Of Acceptance Request. Please note that either whole batch succeeds or fails, there's no partial batch success. To update one Letter Of Acceptance, please pass array with one Letter Of Acceptance object.   Only the creator can edit all the fields when the Letter Of Acceptance in creator inbox(in Rejected status) or in drafts. Update during workflow will have all the fields readonly except few fields which are allowed to modify during each stage of workflow including Workflow Details. The status and workflow state will be updated during each stage of workflow approval. ", response = LetterOfAcceptanceResponse.class, tags = {
            "Letter Of Acceptance", })
    @ApiResponses(value = {
    ...
    @RequestMapping(value = "/letterofacceptances/_update", method = RequestMethod.POST)
