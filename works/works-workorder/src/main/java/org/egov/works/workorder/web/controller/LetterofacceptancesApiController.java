...
@javax.annotation.Generated(value = "io.swagger.codegen.languages.SpringCodegen", date = "2018-02-12T12:32:56.669Z")
...
public class LetterofacceptancesApiController implements LetterofacceptancesApi {
    ...
    public ResponseEntity<LetterOfAcceptanceResponse> letterofacceptancesSearchPost( @NotNull@ApiParam(value = "Unique id for a tenant.", required = true) @RequestParam(value = "tenantId", required = true) String tenantId,
        ...
        @ApiParam(value = "if this value is true, API returns all the LOAs where WorkOrder is created. if this values is false, API returns all the LOAs where WorkOrder is not created. If no value then it returns all the LOAs.") @RequestParam(value = "workOrderExists", required = false) Boolean workOrderExists,
        @ApiParam(value = "if this value is true, API returns all the LOAs where Offline Status is AGREEMENT_ORDER_SIGNED. if this values is false, API returns all the LOAs where Offline Status is not created. If no value then it returns all the LOAs.") @RequestParam(value = "withAllOfflineStatusAndWONotCreated", required = false) Boolean withAllOfflineStatusAndWONotCreated,
        @ApiParam(value = "if this value is true, API returns all the LOAs where Milestone is created. if this values is false, API returns all the LOAs where Milestone is not created. If no value then it returns all the LOAs.") @RequestParam(value = "milestoneExists", required = false) Boolean milestoneExists,
        @ApiParam(value = "if this value is true, API returns all the LOAs where bill is created. if this values is false, API returns all the LOAs where bill is not created. If no value then it returns all the LOAs.") @RequestParam(value = "billExists", required = false) Boolean billExists,
        @ApiParam(value = "if this value is true, API returns all the LOAs where ARF is created. if this values is false, API returns all the LOAs where ARF is not created. If no value then it returns all the LOAs.") @RequestParam(value = "contractorAdvanceExists", required = false) Boolean contractorAdvanceExists,
        @ApiParam(value = "if this value is true, API returns all the LOAs where MB is approved. if this values is false, API returns all the LOAs where MB is not approved. If no value then it returns all the LOAs.") @RequestParam(value = "mbExists", required = false) Boolean mbExists,
        @ApiParam(value = "Returns all LOA where passed offline status is set for LOA.") @RequestParam(value = "offlineStatus", required = false) String offlineStatus,
        @ApiParam(value = "Returns all LOA for passed Loa estimate Id's.") @RequestParam(value = "loaEstimateId", required = false) String loaEstimateId) {
        LetterOfAcceptanceSearchContract letterOfAcceptanceSearchCriteria = LetterOfAcceptanceSearchContract.builder()
                ...
                .statuses(statuses).loaEstimateId(loaEstimateId)
